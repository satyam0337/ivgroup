		...
		if(rateMaster != null && rateMaster.length > 0) {
			for (var i = 0; i < rateMaster.length; i++) {
				...
				if(rateId_mast['cftValue'] == null || (typeof rateId_mast['cftValue'] == 'undefined')) {
					if(Number(rateMaster[i].cftRate) > 0 && Number($('#chargeType').val()) != ChargeTypeConstant.CHARGETYPE_ID_FIX) {
						rateId_mast['cftValue']	= rateMaster[i].cftRate;
					}
				}
			}
		}
...
function getWeightTypeRates() {
	...
	if((Number($("#chargeType").val()) == ChargeTypeConstant.CHARGETYPE_ID_CUBIC_RATE 
			&& configuration.ChargeCubicRateCBMWise == 'true') || (Number($('#chargeType').val()) != ChargeTypeConstant.CHARGETYPE_ID_WEIGHT)) {
		return;
	}
	...
	if (configuration.VolumetricWiseAddArticle == 'true') {
		if($('#actualWeight').val() > $('#chargedWeight').val()) {
			chargedWeight		= $('#actualWeight').val();
		}
	}
...
function getKiloMeterWiseRate() {
	...
	if((Number($("#chargeType").val()) == ChargeTypeConstant.CHARGETYPE_ID_CUBIC_RATE 
			&& configuration.ChargeCubicRateCBMWise == 'true') || (Number($('#chargeType').val()) != ChargeTypeConstant.CHARGETYPE_ID_KILO_METER)) {
		return;
	}
