...
define([ 
		...
		,'jquerylingua'
         ,'language'
         ,'/ivcargo/resources/js/module/view/fuelHisabVoucherDetailsReport/fuelHisabVoucherDetailsReportfilepath.js'
         ,'focusnavigation'
         ,'nodvalidation'
         ,'slickGridWrapper3'
         ,'autocomplete'
	...
	return Marionette.LayoutView.extend({
		initialize : function(masterObj) {
			...
			getJSON(null, WEB_SERVICE_URL + '/fuelHisabVoucherDetailsReportWS/getFuelHisabVoucherDetailsReportElement.do?',	_thisRender.loadViewForReport, EXECUTE_WITHOUT_ERROR);
			return _thisRender;
			...
			$("#mainContent").load( "/ivcargo/html/report/fuelHisabVoucherDetailsReport/fuelHisabVoucherDetailsReport.html" , function() {
				baseHtml.resolve();
			});
			...
			getJSON(inJson, WEB_SERVICE_URL + '/fuelHisabVoucherDetailsReportWS/getFuelHisabVoucherDetailsReport.do?', _thisRender.setDataInTable, EXECUTE_WITH_ERROR);
			...
			var ColumnConfig		= response.fuelHisabVoucherDetailsReportList.columnConfiguration;
			...
			response.fuelHisabVoucherDetailsReportList.columnConfiguration	= _.values(bcolConfig);
			response.fuelHisabVoucherDetailsReportList.Language				= LangKeySet;
			
			slickGridWrapper3.applyGrid({
				ColumnHead					: response.fuelHisabVoucherDetailsReportList.columnConfiguration, // *compulsory // for table headers
				ColumnData					: _.values(response.fuelHisabVoucherDetailsReportList.CorporateAccount), 	// *compulsory // for table's data
				Language					: response.fuelHisabVoucherDetailsReportList.Language, 			// *compulsory for table's header row language
...
function transportSearch(grid,dataView,row){
	var SEARCH_TYPE_ID_FUEL_HISAB_VOUCHER_NUMBER		= 18;
	if(dataView.getItem(row).fuelHisabVoucherId != undefined && dataView.getItem(row).fuelHisabVoucherId > 0) {
		window.open('TransportSearchWayBillAction.do?pageId=5&eventId=3&wayBillId='+dataView.getItem(row).fuelHisabVoucherId+'&wayBillNumber='+dataView.getItem(row).fuelHisabVoucherNumber+'&TypeOfNumber='+SEARCH_TYPE_ID_FUEL_HISAB_VOUCHER_NUMBER+'&BranchId=0'+'&selectedVehicleNoId='+dataView.getItem(row).vehicleNumberMasterId+'&searchVehicle='+dataView.getItem(row).vehicleNumber);
	} 
}
