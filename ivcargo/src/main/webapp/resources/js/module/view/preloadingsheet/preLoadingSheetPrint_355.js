define(['elementTemplateJs'
	...
	sortByLastCharWithoutBranchSource	= false,
	...
	return ({
		renderElements : function(response){
			...
			sortByLastCharWithoutBranchSource = response.sortByLastCharWithoutBranchSource;
			...
			require(['text!/ivcargo/template/preloading/preloadingsheet_24.html'],function(PreLoadingSheet){
				...
				for(var i = 0;i<finalJsonObj.length;i++){
					...
					if(pendingDispatchModel.pendingQuantity > 0){
						...
						totalChargeWeight += pendingDispatchModel.consignmentSummaryChargedWeight;
						totalBookingTotal += pendingDispatchModel.bookingTotal;
						totalBookingFreight += pendingDispatchModel.wayBillFreightCharge;
						totalDoorPickup += pendingDispatchModel.wayBillDoorPickupCharge;
						totalDoorDelivery += pendingDispatchModel.wayBillDoorDeliveryCharge;
						totalOther	+= pendingDispatchModel.wayBillOtherCharge;
						totalNoOfLR++;
					}
				}
				var pageCount;
				for(var key in chunkArray){
					...
					pageCount = parseInt(key)+parseInt(1);
				...
				$('.chargeweight').html(totalChargeWeight);
				...
				$('.divTable .divTableFoot:not(:last)').each(function(){$(this).empty()});
				hideLayer();	
			...
			$("*[data-branch='name']").html('Branch Name : ' +headerCollection.branchName);
			$("*[data-branch='address']").html(headerCollection.branchAddress);
			$("*[data-branch='phonenumber']").html('Phone No. : ' +headerCollection.branchContactDetailPhoneNumber);
			$("*[data-current='datetime']").html('Date :' + currentDateTime);
			...
			//lastcharacterwise sorting
			if(sortByLastCharWithoutBranchSource == 'true' || sortByLastCharWithoutBranchSource == true )
				{
			...
			for(var i=0;(wayBillArray).length > i;i++) {
				...
				wayBillArray[i].lastCharWayBillNumber	= wayBillNumber.substring(wayBillNumber.length - 1,wayBillNumber.length);
			}
			var detsinationcollection = _.sortBy(wayBillArray,'lastCharWayBillNumber');
