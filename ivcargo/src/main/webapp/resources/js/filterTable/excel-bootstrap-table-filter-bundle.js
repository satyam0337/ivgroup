...
var FilterCollection = function () {
	...
	FilterCollection.prototype.sort = function (column, order, table, options) {
		...
		if('incomingDateTimeStampString' == rows[0].children[column].id.split('_')[0]){
			rows.sort(function(a,b) {
				if(flip==1)
					return convertDate(a.cells[column].innerHTML) - convertDate(b.cells[column].innerHTML);
				else
					return convertDate(b.cells[column].innerHTML) - convertDate(a.cells[column].innerHTML);
			});
		} else {
			rows.sort(function (a, b) {
