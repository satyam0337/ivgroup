define (['language',
	...
	return ({
		setDataForView : function (responseObj){
			...
			$.when.apply($, loadelement).done(function() {
				...
				_.mapObject(collection,function(groupData,key){
					...
					var keys	= Object.keys(dispatchData[key]);
				...
				for(var key in accountGroupKey){
					_this.setHeaderWiseData(accountGroupKey.length,groupDataObject[accountGroupKey[key]],dispatchDataAccountGroupWise[accountGroupKey[key]],dispatchDataAccountGroupWise[accountGroupKey[key]],dispatchLSLRCharge,dispatchLSHeaderAccountGroupWise[accountGroupKey[key]],dispatchLSLRFormSummaryAccountGroupWise[accountGroupKey[key]],showPartialLSDetailsSummary,displayData,lhpvChargeList,printType);
				}
				hideLayer();
				if(printType == null){
					setTimeout(function(){
		...
		}, setHeaderWiseData : function (totalGroup,groupData,contentCollection,dispatchDataAccountGroupWise,dispatchLSLRCharge,dispatchLSHeaderAccountGroupWise,dispatchLSLRFormSummaryAccountGroupWise,showPartialLSDetailsSummary,displayData,lhpvChargeList,printType){
			...
			if(printType == 'printSummary' ){
				...
				_this.setTotalSummary(groupData,groupHeaderWiseTable,displayData,dispatchLSLRFormSummaryAccountGroupWise,partialCollection,lhpvChargeList,destinationWiseCollection);
				...
				_this.setSummaryOfDestination(groupData,groupHeaderWiseTable,displayData,dispatchLSLRFormSummaryAccountGroupWise,destinationWiseCollection,partialCollection);
				_this.setHeader(dispatchLSHeaderAccountGroupWise,groupData,dispatchLSLRCharge,tableObjTr,groupHeaderWiseTable,dispatchDataAccountGroupWise,displayData,dispatchLSLRFormSummaryAccountGroupWise,partialCollection,lhpvChargeList,destinationWiseCollection);
			...
			while(dataTableRows.length > 0){
				...
				if(groupData.accountGroupName.split(" ")[0] == 'Bombay'){
					...
					var chunkArray1				= dataTableRows.splice(0,rowsPerPage);//.map(key => ({chunkArray : chunkArray[0]}));
					var chunkArray = Object.values(chunkArray1);
						//$.map(chunkArray1, function(value, index) {
					    //return [value];
					//});
					/*var chunkArray = Object.keys(chunkArray1).map(function(key) {
						  return [Number(key), chunkArray1[key]];
						});*/
				//}
				...
				for(var key in chunkArray){
					...
					if(typeof chunkArray[key] != "function"){
						if(adjustRows > key){
							...
							if(chunkArray[key] != undefined && chunkArray[key].children().length == 6){
			...
			if(totalRow > (rowsPerPageSummary - 2) && pageWiseHeader){
				...
				for(var i = 0; i < columnObjectForDetails.length ; i++){
					...
					if(datapiker == 'branchName'){
						newtd.append(groupData.branchName);
						newTr.append(newtd);
					}
					if(datapiker == 'branchAddress'){
						newtd.append(groupData.branchAddress);
						newTr.append(newtd);
					}
					if(datapiker == 'branchPhoneNumber'){
						newtd.append(groupData.branchContactDetailPhoneNumber);
						newTr.append(newtd);
					}
		...
		}, setTotalSummary : function(groupData,groupHeaderWiseTable,displayData,dispatchLSLRFormSummaryAccountGroupWise,partialCollection,lhpvChargeList,destinationWiseCollection){
			totalRow++;
				var keyObject							= Object.keys(destinationWiseCollection); 
				var destinationData						= new Object();
				var totalnoOfLr							= 0; 
		...
		},setSummaryOfDestination : function (groupData,groupHeaderWiseTable,displayData,dispatchLSLRFormSummaryAccountGroupWise,destinationWiseCollection,partialCollection){
			totalRow++;
			var keyObject							= Object.keys(destinationWiseCollection); 
			...
			for(var key in keyObject){
				...
				_.each(dispatchDetails,function(waybillIdData){
					...
					if(waybillIdData.amountShow){
						if(waybillIdData.wayBillTypeId == WayBillTypeConstant.WAYBILL_TYPE_TO_PAY){
			...
			for(var key in keyObject){
				...
				_.each(dispatchDetails,function(dispatchDeta){
					...
					totalactualWeight		+= dispatchDeta.wayBillActualWeight;
					totalnoOfArticle		+= dispatchDeta.wayBillArticleQuantity;
					
					if(dispatchDeta.wayBillTypeId != WayBillTypeConstant.WAYBILL_TYPE_TO_PAY){
						totalRow++;
			...
			_.each(partialCollection,function(partialDetails){
				...
				if(partialDetails.wayBillTypeId != WayBillTypeConstant.WAYBILL_TYPE_TO_PAY){
