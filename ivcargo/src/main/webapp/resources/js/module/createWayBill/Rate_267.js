...
function calculateChargedWeight(obj) {
	...
	corporateAccountId	= getPartyIdForRate();
		...
		if(rateMaster != null && rateMaster.length > 0) {
			for (var i = 0; i < rateMaster.length; i++) {
				if(rateId_mast[rateMaster[i].chargeTypeMasterId] == null || (typeof rateId_mast[rateMaster[i].chargeTypeMasterId] == 'undefined') || rateId_mast[rateMaster[i].chargeTypeMasterId] == 0) {
					...
					} else if((rateMaster[i].minWeight == 0) && (rateMaster[i].maxWeight == 0)){
						if(isSlabRateNotExists){
							isSlabRateNotExists = false;
							if(rateId_mast[rateMaster[i].chargeTypeMasterId] >= 0) {
								rateId_mast[rateMaster[i].chargeTypeMasterId] = rateMaster[i].rate;
...
function calculateWeightTypeRates(chargedWeight, typeofPacking, consignmentGoodsId) {
	...
	if(billingPartyId > 0) {
        ...
        if(configuration.ChargeTypeFlavour == '1') {
            ...
            categoryTypeId        = RateMaster.CATEGORY_TYPE_PARTY_ID;
        } else if(configuration.ChargeTypeFlavour == '2') {
	...
	findAllSlab(quantity,corporateAccountId,wayBillTypeId,typeofPacking,consigneeId,vehicleTypeId,wayBillTypeId);
	...
	for (var key in response) {
		...
		if(configuration.ApplyArticleWiseRateOnWeightType == 'true' && getBookingType() == BookingTypeConstant.BOOKING_TYPE_SUNDRY_ID
			...
			if(chargeType == ChargeTypeConstant.CHARGETYPE_ID_QUANTITY) {
				...
				&& jQuery.inArray(chargeTypeId + "", aplyRateForCharges) != -1 && (chargeType != ChargeTypeConstant.CHARGETYPE_ID_FIX)) { //aplyRateForCharges coming from Property
			qtyAmt	= Number(response[key]) * Number(quantity);
		} else {
			qtyAmt	= Number(response[key]);
		}
...
function partyMinimumRates() {
	...
	var corporateAccountId	= getPartyIdForRate();
	var minAmt					= 0;
...
function calculateWeightAndQuantityWiseFreightAmount(chargeMasterId) {
	...
	if(fixTypeArr != null) { // fix
		fixAmnt			= getAmntFromWeightAmntArr(chargeMasterId, fixTypeArr);
	}
	...
	if(fixAmnt > 0){
		$('#charge' + chargeMasterId).val(Number(fixAmnt));
		
		if(chargeMasterId == ChargeTypeMaster.FREIGHT) {
			$('#freightChargeValue').val(Number(fixAmnt));
		}
	}else{
...
function applyRates() {
	...
	if(calcDDChargeArr != null && calcDDChargeArr != undefined) {
		var ddcAmount = 0;
		var deliveryTo = $('#deliveryTo').val();
		for(var i = 0; i < calcDDChargeArr.length; i++){
			ddcAmount += Number(calcDDChargeArr[i].split("=")[1]);
		}
		if(deliveryTo == InfoForDeliveryConstant.DELIVERY_TO_DOOR_ID){
			$('#charge'+BookingChargeConstant.DDC).val(Number(ddcAmount));
		}else{
			$('#charge'+BookingChargeConstant.DDC).val(0);
		}
...
function getAmntFromWeightAmntArr(chargeMasterId,weightTypeArr){
	...
	if($('#chargeType').val() == ChargeTypeConstant.CHARGETYPE_ID_FIX && chargeMasterId == BookingChargeConstant.FREIGHT && amount == 0) {
		var fixAmt = 0;
...
//applied various condition for fix type rates form rate master
// getWghtTypeRates 
function getFixChargeTypeRates(srcBranchId, destBranchId, vehicleTypeId, corporateAccountId, categoryTypeId, packingTypeId, packingGrpTypeId, userWeightInput, rateTypeId, consignmentGoodsId, consigneeId) {

	var chargeTypeId		= ChargeTypeConstant.CHARGETYPE_ID_FIX;
	var rateId_mast			= new Object();
	var rateMaster			= null;
	var chargeSectionId		= 0;
	isSlabRateNotExists 	= false;
...
function calculateFixTypeRates(chargedWeight, typeofPacking, consignmentGoodsId) {
	if(configuration.applyRateAuto == 'false' || $('#wayBillType').val() == WayBillType.WAYBILL_TYPE_FOC) {
		return;
	}
	...
	if($('#wayBillType').val() != WayBillType.WAYBILL_TYPE_FOC) {
		var response =  getFixChargeTypeRates(srcBranchId, destBranchId, vehicleTypeId, corporateAccountId, categoryTypeId, typeofPacking, packingGrpTypeId, chargedWeight, rateTypeId, consignmentGoodsId, consigneeId);
		if(response && !jQuery.isEmptyObject(response)) {

			fixFromDb		= 0;
			fixFromDb 		= getFreightChargeForRates(response);
			var chargedWeight 	= Number($('#chargedWeight').val());
			
			$('#weigthFreightRate').val(0);
			$('#weightAmount').val(0);
			$('#fixAmount').val(fixFromDb);
...
function getFixTypeRates() {
	...
	var fixAmnt			= calculateFixTypeRates(chargedWeight, typeofPacking, consignmentGoodsId);
	$('#fixAmount').val(fixAmnt);
	
}
function getFixAmountNotChange() {
	var fixAmount = $('#fixAmount').val();
	if((Number(fixAmount > fixFromDb || fixAmount < fixFromDb)) && fixFromDb != 0){
		//showMessage('info', "You Can Not Change Existing Rate !");
		//$('#fixAmount').val(fixFromDb);
	}
}

function getPartyIdForRate() {
	var corporateAccountId	= 0;
...
function findAllSlab(quantity,corporateAccountId,wayBillTypeId,typeofPacking,consigneeId,vehicleTypeId,wayBillTypeId){
	//var userInput		= $('#quantity').val();
	var slabMinRangeHM = new Map();
	var noneSlab = 0;
	var dccCharge = 0;
	var ddcString = "";
	
	var response =  getchargeTypeRatesByCharges(quantity,corporateAccountId,typeofPacking,vehicleTypeId,consigneeId,wayBillTypeId);
	if(response && !jQuery.isEmptyObject(response)) {
				for(var i = 0; i < response.length; i++) {
				var wayBillRates	=	 response[i];
				noneSlab = 0;
				if(wayBillRates.slabMasterId == 0 && wayBillRates.chargeTypeMasterId == BookingChargeConstant.DDC){
			...
			for(var i = 0; i < response.length; i++) {
				var wayBillRatesObejct	= response[i];
				var	chargeMasterId	= wayBillRatesObejct.chargeTypeMasterId;
				var checkChargeId = configuration.doorDeliveryChargeId == chargeMasterId;
				
				if(wayBillRatesObejct.slabMasterId > 0 && quantity >= wayBillRatesObejct.minWeight && quantity <= wayBillRatesObejct.maxWeight) {
					if(checkChargeId){
						if(!calcDDCharge) {
							dccCharge = wayBillRatesObejct.rate;
							calcDDCharge	= true;
							
							if(i == 0) {
								ddcString += BookingChargeConstant.DDC+"="+dccCharge;
							} else {
								ddcString += ","+BookingChargeConstant.DDC+"="+dccCharge;
							};
							
							if(ddcString != "")
								$('#ddcCharge').val(ddcString);
						}
					}
				...
				if(slabMinRangeHM.size > 0) {
					...
					for (const [key, value] of slabMinRangeHM.entries()) {
						...
						if(maxValue == value) {
							if(key == wayBillRatesObejct.slabMasterId) {
								if(checkChargeId){
									var diff = Number(quantity - maxValue);
									dccCharge = wayBillRatesObejct.rate + (diff * noneSlab);
...
function getchargeTypeRatesByCharges(quantity,corporateAccountId,typeofPacking,vehicleTypeId,consigneeId,wayBillTypeId) {

	var rateMaster			= null;
	isSlabRateNotExists 	= false;
	var rateMasterArr		= new Array();

	rateMaster = getfilterRatesByCharges(quantity,corporateAccountId,typeofPacking,0,consigneeId,wayBillTypeId);
	checkAndInsertRatesByCharges(rateMaster, quantity,rateMasterArr);
	
	rateMaster = getfilterRatesByCharges(quantity,corporateAccountId,typeofPacking,0,consigneeId, 0);
	checkAndInsertRatesByCharges(rateMaster, quantity,rateMasterArr);
	
	//Route level charges
	rateMaster = getfilterRatesByCharges(quantity,corporateAccountId,typeofPacking,0,0,wayBillTypeId);
	checkAndInsertRatesByCharges(rateMaster, quantity,rateMasterArr);
	
	rateMaster = getfilterRatesByCharges(quantity,corporateAccountId,typeofPacking,0,0,0);
	checkAndInsertRatesByCharges(rateMaster, quantity,rateMasterArr);
	
	return rateMasterArr;
}
function getfilterRatesByCharges(quantity,corporateAccountId,typeofPacking,vehicleTypeId,consigneeId,wayBillTypeId) {
	var accountGroupId	= executive.accountGroupId;
	var arr 			= new Array();
	...
	if(!jQuery.isEmptyObject(wayBillRates)) {
		for (var i = 0; i < wayBillRates.length; i++) {
			...
			if(Number(datarates.accountGroupId) 				== Number(accountGroupId)
					&& Number(datarates.vehicleTypeId) 			== Number(vehicleTypeId)
					&& Number(datarates.packingTypeId) 			== Number(typeofPacking)
					&& Number(datarates.corporateAccountId) 	== Number(corporateAccountId)
					&& Number(datarates.wayBillTypeId) 			== Number(wayBillTypeId)
					&& Number(datarates.consigneePartyId) 		== Number(consigneeId)
			) {
...
function checkAndInsertRatesByCharges(rateMaster , userInput,rateMasterArr) {
	if(rateMaster != null && rateMaster.length > 0) {
		for (var i = 0; i < rateMaster.length; i++) {
