define(['elementTemplateJs'
	...
	var destBranchCount;
	var totalNoOfQuantity = 0, totalActualWeight = 0, totalTopayAmount = 0;
	return ({
		renderElements : function(response) {
			...
			totalNoOfQuantity 			= response.totalNoOfQuantity;
			totalActualWeight 			= response.totalActualWeight;
			totalTopayAmount 			= response.totalTopayAmount;
			...
			require(['text!/ivcargo/template/preloading/' + flavourType + '.html'], function(PreLoadingSheet) {
				...
				$('.totalTopayAmount').last().html(totalTopayAmount);
				$('.grandTotal').last().html("Grand Total");
