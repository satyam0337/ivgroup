        ...
        renderElements : function(){
			...
			$('#popUpContent628').bPopup({
						...
						$(this).html("<div class='confirm' style='height:180px'><h1>Print option</h1><br><center>" +
							...
							"<input type='checkbox' name='showHeader' id='showHeader' style='font-weight: bold;font-size: 20px;' unchecked />Show Header</center> " +
						...
						$("#printCharges").click(function() {
							if ($("#setAmount").is(":checked") && $("#showHeader").is(":checked")) {
								$(".printHeader628").show();
								$(".printHeight").hide();
								$("#jktcLrPrintTable1").css({ 'margin-top': '10px' });
								$("#jktcLrPrintTable2").css({ 'margin-top': '10px' });
								$("#jktcLrPrintTable3").css({ 'margin-top': '10px' });
								_thisMod.close();
								_this.printWindow(isPdfExportAllow);
							} else if ($("#setAmount").is(":checked")) {
								_thisMod.close();
								_this.printWindow(isPdfExportAllow);
							} else if ($("#showHeader").is(":checked")) {
								$(".printHeader628").show();
								$(".printHeight").hide();
								$("#jktcLrPrintTable1").css({ 'margin-top': '10px' });
								$("#jktcLrPrintTable2").css({ 'margin-top': '10px' });
								...
								_thisMod.close();
								_this.printWindow(isPdfExportAllow);
							} else if ($("#setAmount").not(":checked") && $("#showHeader").not(":checked")) {
								_this.setZeroAmount(chargeTypeModelArr);
								_thisMod.close();
								_this.printWindow(isPdfExportAllow);
							}
