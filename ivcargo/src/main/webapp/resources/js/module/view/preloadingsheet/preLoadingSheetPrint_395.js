define(['elementTemplateJs'
	...
	return ({
		renderElements : function(response){
			...
			require(['text!/ivcargo/template/preloading/preloadingsheet_27.html'],function(PreLoadingSheet){
				...
				for(var key in chunkArray){
					...
					$('.currentDateTime2').html(currentDateTime);
					$('.emptySpace').html(emptySpace);
				}
			...
			for(var chunk in chunkArray){
				$.each( $(className).last(), function(i, left) {
					$('div', left).each(function() {
						$("*[data-cell='emptyCell']").html(emptySpace);
