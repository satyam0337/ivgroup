define(['elementTemplateJs'
	...
	allowBranchIdWisePreloadingSheetPrint = false,
	...
	return ({
		renderElements : function(response){
			...
			allowBranchIdWisePreloadingSheetPrint	= response.allowBranchWisePreloadingSheetPrint;
			...
			if(dataView == undefined){
				if(allowBranchIdWisePreloadingSheetPrint == 'true' || allowBranchIdWisePreloadingSheetPrint == true ) {
					require(['text!/ivcargo/template/preloading/preloadingsheetBranchWise_31.html'],function(PreLoadingSheet){
						_this.setData(finalJsonObj,PreLoadingSheet);
					})
				}else {
					require(['text!/ivcargo/template/preloading/preloadingsheet_31.html'],function(PreLoadingSheet){
						_this.setData(finalJsonObj,PreLoadingSheet);
					})
				}
			}else{
		...
		renderTable:function(chunkArray){
			if(allowBranchIdWisePreloadingSheetPrint == 'true' || allowBranchIdWisePreloadingSheetPrint == true ) {
				var leftDataChunk = chunkArray.splitArray(Math.ceil(chunkArray.length/3));
				for(var leftData in leftDataChunk){
					if(leftData == 0){
						this.renderTableValues(leftDataChunk[leftData],'.leftdivTableBody');
					}else if(leftData == 1){
						this.renderTableValues(leftDataChunk[leftData],'.middledivTableBody');
					}else {
			...
			$(".loadingArticle").css({'color':'white'});
