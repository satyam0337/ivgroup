	...
	if(data.pendingShSettlementCall) {
		...
		if(pendingShSettlement.length > 0) {
			for(var i = 0; i < pendingShSettlement.length; i++) {
				...
				var totalValue		= pendingShortReceive.declaredValue;
				var articleQuantity	= pendingShortReceive.quantity;
				var shortValue		= pendingShortReceive.shortValue;
				var destBranch		= pendingShortReceive.lrDestinationBranch;
				var srcBranch		= pendingShortReceive.lrSourceBranch;
				var turBranchId		= pendingShortReceive.turBranchId;
				var turBranchName	= pendingShortReceive.turBranchName;
				...
				if(accountGroupId == accountGroupLMT){
					...
					var srcBranchCol		= createNewColumn(row, 'srcBranch_s'+(i+1), 'datatd', '', '', '', '');
					var destBranchCol		= createNewColumn(row, 'destBranch_s'+(i+1), 'datatd', '', '', '', '');
					...
					var totalValueCol		= createNewColumn(row, 'totalValue_s'+(i+1), 'datatd', '', '', '', '');
					var shortValueCol		= createNewColumn(row, 'shortValue_s'+(i+1), 'datatd', '', '', '', '');
					...
					$(shortValueCol).append(shortValue);
				...
				if(configuration.isTotalValueColumnDisplay == 'false') {
					changeDisplayProperty('totalValue_s'+(i+1), 'none');
				}
				if(configuration.isShortValueColumnDisplay == 'false') {
					changeDisplayProperty('shortValue_s'+(i+1), 'none');
				}
				if(configuration.isSourceBranchColumnDisplay == 'false') {
					changeDisplayProperty('srcBranch_s'+(i+1), 'none');
				}
				if(configuration.isDestinationBranchColumnDisplay == 'false') {
					changeDisplayProperty('destBranch_s'+(i+1), 'none');
				}
			}
			...
			if(configuration.isTotalShortRowDisplay == 'true') {
				if(accountGroupId == accountGroupLMT){
					if(data.valueObjectForTotal) {
						...
						if(valueObjectForTotal.totalShortValue) {
							totalShortValue	= valueObjectForTotal.totalShortValue;
						}
						...
						if(configuration.isTotalArticleColumnDisplay != undefined && configuration.isDamageArticleColumnDisplay == 'true'){
						...
						var blankCol106			= createNewColumn(totalRow, '', 'titletd', '', '', 'background-color: #F5DEB3; font-weight: bold', '');
						var blankCol107			= createNewColumn(totalRow, '', 'titletd', '', '', 'background-color: #F5DEB3; font-weight: bold', '');
						...
						var blankCol108			= createNewColumn(totalRow, '', 'titletd', '', '', '', '');

						$(totalCol).append('Total');
						$(blankCol1).append();
						$(blankCol2).append();
						$(blankCol202).append();
						if(configuration.isTotalArticleColumnDisplay != undefined && configuration.isTotalArticleColumnDisplay == 'true') {
						...
						$(blankCol104).append();
						$(blankCol105).append();
						$(blankCol106).append(totalDeclaredValue);
						$(blankCol107).append(totalShortValue);
						$(blankCol108).append();
