/**
 * @Author	Shailesh Khanadare 21-01-2016
 */

function getAllDamageSettlementList() {
	...
	//alert(jsonStr);
	showLayer();
	$.getJSON("getAllDamageReceiveSettlementDetails.do?pageId=333&eventId=9", 
			{json:jsonStr}, function(data) {
				...
				if (!data || jQuery.isEmptyObject(data) || data.errorDescription) {
					...
					removeTableRows('damageSettleReport', 'tbody');
					removeTableRows('printDamageSettlementReport', 'table');

					if (data.dmSettlementReportCall) {

						var dmoSettleReportData = data.dmSettlementReportCall;

						if (dmoSettleReportData.length > 0) {

							for (var i = 0; i < dmoSettleReportData.length; i++) {
								dmoSettleReport = dmoSettleReportData[i];

								var srNo 				= (i + 1);
								var wayBillNumber 		= dmoSettleReport.wayBillNumber;
								var wayBillId			= dmoSettleReport.wayBillId;
								var shortDate 			= dmoSettleReport.damageDate;
								var damageReceiveId		= dmoSettleReport.damageReceiveId;
								var shortBranch 		= dmoSettleReport.damageBranch;
								var shortUser 			= dmoSettleReport.damageUser;
								var settlementDate 		= dmoSettleReport.damageSettleDate;
								var settlementNumber 	= dmoSettleReport.settlementNumber;
								var settlementBranch 	= dmoSettleReport.settlementBranch;
								var settlementUser 		= dmoSettleReport.settlementUser;
								var settlementType 		= dmoSettleReport.settlementType;
								var remark 				= dmoSettleReport.remark;
								var damageNumber		= dmoSettleReport.damageNumber;
								...
								$(shortNumberCol).append('<a href="#" onmouseover="onMouseOverDisplayShortArt('+damageReceiveId+')" onmouseout="onMouseOutHideShortArt('+damageReceiveId+')">'+ damageNumber+ '</a>');
								...
								if(isShortArticleDetailsDisplay != 'true') {
									...
									shortArticleDetails(data, damageReceiveId);
								}

								if (settlementType == SETTLE_WITH_CLAIM) {
								...
								$("#damageSettleReport").append(row);
								
								 if (settlementType == SETTLE_WITH_CLAIM) {

									claimNumber = dmoSettleReport.claimNumber;
									...
									$("#damageSettleReport").append(claimRow);
									...
									$("#damageSettleReport").append(noClaimRow);
						...
						printTable(data, 'reportData', 'damageRegisterSettlementReport', 'Damage Register Settlement Summary', 'printDamageSettlementReport');
	...
	if (data.excessReceiveHMCall) {
		...
		if (exhm.hasOwnProperty(excessNumber)) {
			...
			for (var i = 0; i < list.length; i++) {
				...
				$(excessNumberValueCol).append('<a href="#" onmouseover="onMouseOverShowExcessArt('+excessNumber+')" onmouseout="onMouseOutHideExcessArt('+excessNumber+')">' + excessNumber + '</a>');
