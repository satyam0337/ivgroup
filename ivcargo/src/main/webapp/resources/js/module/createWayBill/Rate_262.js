...
function refreshSession(){
	...
	var startTimeHour			= jsondata.startTimeHour;
	var startTimeMinute			= jsondata.startTimeMinute;
	var endTimeHour				= jsondata.endTimeHour;
	var endTimeMinute			= jsondata.endTimeMinute;
	var noRecords				= jsondata.noRecords;
	var discountTimeList    	= jsondata.discountTimeList;
	var startTimeMilisecond     = 0;
	var endTimeMilisecond	    = 0;
	var currentTimeMilisecond   = 0;
	currentTimeMilisecond       = curDate.getTime();
	
	if(noRecords == false && discountTimeList != undefined){
		for(var j=0;j<discountTimeList.length;j++){
			
			startTimeMilisecond = discountTimeList[j].startTimeMilisecond;
			endTimeMilisecond   = discountTimeList[j].endTimeMilisecond;
			
			/*if(noRecords == false){
				if((curDate.getHours() >= startTimeHour && curDate.getHours() <= endTimeHour && curDate.getMinutes() >= startTimeMinute && curDate.getMinutes() < endTimeMinute) && (DiscountMaster == undefined || DiscountMaster == 'undefined')){
					if(confirm("Happy Hours are Started, Please Press OK to apply Discount.")){
						location.reload();
					}
				}else if((startTimeHour <= curDate.getHours() && curDate.getHours() >= endTimeHour && startTimeMinute <= curDate.getMinutes() && curDate.getMinutes() >= endTimeMinute) && (DiscountMaster != undefined && DiscountMaster != 'undefined')){
					if(confirm("Happy Hours are Ended, Please Press OK to remove Discount.")){
						location.reload();
					}
				}
			}*/
...
function checkRateForPackingType(qtyAmount, quantity ) {
	...
	if(quantityWiseChargeAmount == null) {
		quantityWiseChargeAmount	= {};
		finalQtyAmt = calculateChargeAmount(configuration.QtyFlavor, qtyAmount, quantity);
		quantityWiseChargeAmount[BookingChargeConstant.FREIGHT] = parseFloat(finalQtyAmt);
	} else {
		for (let [key, value] of Object.entries(quantityWiseChargeAmount)) {
			if(key == BookingChargeConstant.FREIGHT) {
                value = calculateChargeAmount(configuration.QtyFlavor, qtyAmount, quantity);
                isFreightRateExist = true;
                quantityWiseChargeAmount[key] = value;
                break;
            }
		}
		
		if(!isFreightRateExist) {
            var finalQtyRate 	= calculateChargeAmount(configuration.QtyFlavor, qtyAmount, quantity);
            quantityWiseChargeAmount[BookingChargeConstant.FREIGHT] = finalQtyRate;
        }
	}
	...
	for (var i = 0; i < chargeTypeModel.length; i++) {
		...
		if(applicableChargeRateObj[chargeMasterId]) {
			quantityWiseChargeAmount[chargeMasterId] = calculateApplicableAmount(chargeMasterId, quantityWiseChargeAmount);
		}
	}
}
...
function checkIfnotPresent() {
	...
	if (configuration.VolumetricWiseAddArticle == 'true') {
		if(Number($('#actualWeight').val()) > Number($('#chargedWeight').val())) {
	...
	for (let [chargeTypeId, value] of Object.entries(weightWiseChargeAmount)) {
		if(chargeTypeId == BookingChargeConstant.FREIGHT) {
			value = calculateChargeAmount(configuration.WghtFlavor, weigthFreightRate, chargedWeight);
			isFreightRateExist = true;
		} else {
		...
		weightWiseChargeAmount[chargeTypeId] = value;
	}

	if(!isFreightRateExist) {
		var finalWghtRate = calculateChargeAmount(configuration.WghtFlavor, weigthFreightRate, chargedWeight);
		
		weightWiseChargeAmount[BookingChargeConstant.FREIGHT] = finalWghtRate;
	}
	...
	for (var i = 0; i < chargeTypeModel.length; i++) {
		...
		if(applicableChargeRateObj[chargeMasterId]) {
			weightWiseChargeAmount[chargeMasterId] = calculateApplicableAmount(chargeMasterId, weightWiseChargeAmount);
		}
	}
...
function calculateChargedWeight(obj) {
	...
	if(configuration.IsDisplayAlertToSelectLCV == 'true') {
		if(Number(actualWeight) >= configuration.MaxWeightToShowAlertForLCV && getBookingType() == configuration.OnBookingTypeToShowAlertForLCV) {
	...
	case 1:
		if(rateMaster != null && rateMaster.length > 0) {
			for (var i = 0; i < rateMaster.length; i++) {
				...
				if((applicableChargeRateObj[rateMaster[i].chargeTypeMasterId] == null
						|| typeof applicableChargeRateObj[rateMaster[i].chargeTypeMasterId] == 'undefined')
						&& rateMaster[i].chargedPercent && rateMaster[i].applicableOn > 0) {
					applicableChargeRateObj[rateMaster[i].chargeTypeMasterId] = rateMaster[i];
				}
			}
		}
		break;
...
function calculateWeightTypeRates(chargedWeight, typeofPacking, consignmentGoodsId) {
	...
	if($('#wayBillType').val() != WayBillType.WAYBILL_TYPE_FOC) {
		...
		if(response && !jQuery.isEmptyObject(response)) {
			...
			if(weightTypeForRateApply == ChargeTypeConstant.WEIGHT_TYPE_ID_ACTUAL_WEIGHT) {
				getTempStringForRates(response, actualWeight, Number($("#chargeType").val()));
			} else {
				getTempStringForRates(response, chargedWeight, Number($("#chargeType").val()));
			}
		} else {
			weightWiseChargeAmount	= {};
		...
		if(configuration.AllowToBookTbbLrWithZeroAmountIfRateNotDefined == 'true' || configuration.AllowToBookTbbLrWithZeroAmountIfRateNotDefined == true) {
			if($('#wayBillType').val() == WayBillType.WAYBILL_TYPE_CREDIT) {
				if($('#chargeType').val() == ChargeTypeConstant.CHARGETYPE_ID_WEIGHT
						|| $('#chargeType').val() == ChargeTypeConstant.CHARGETYPE_ID_KILO_METER){
...
function getTempStringForRates(response, quantity, chargeTypeId) {
	...
	for (var key in response) {
		...
		if(configuration.ApplyArticleWiseRateOnWeightType == 'true' && getBookingType() == BookingTypeConstant.BOOKING_TYPE_SUNDRY_ID
			...
			} else if(chargeTypeId == ChargeTypeConstant.CHARGETYPE_ID_WEIGHT) {
				if(actualWeight > 0) {
					qtyAmt	= Number(response[key]) * Number(totalQty);
				} else {
					qtyAmt	= Number(response[key]) * Number(quantity);
				}
			} else if(chargeTypeId == ChargeTypeConstant.CHARGETYPE_ID_KILO_METER) {
			...
			if(chargeTypeId == ChargeTypeConstant.CHARGETYPE_ID_KILO_METER)
		...
		if(chargeTypeId == ChargeTypeConstant.CHARGETYPE_ID_QUANTITY)
			quantityWiseChargeAmount[chargeMasterId] = qtyAmt;
		else
			weightWiseChargeAmount[chargeMasterId] = qtyAmt;
	}
	...
	console.log("weightWiseChargeAmount --> ", weightWiseChargeAmount)
}
...
function chargeDisableFromRate(){
	...
	var charges	=  chargesToApplyArticleWiseRateOnWeightType;
			if(configuration.ApplyArticleWiseRateOnWeightType == 'true' || configuration.ApplyArticleWiseRateOnWeightType == true){
					for (var i = 0; i < charges.length; i++) {
						var chargeMasterId	= charges[i];
						if($('#charge' + chargeMasterId).val() > 0){
						$('#charge' + chargeMasterId).prop("readonly", "true");
						}
					}
			}
...
function calculatePackingTypeRates(quantity, typeofPacking,consignmentGoodsId){
	...
	if(typeofPacking > 0 && $('#wayBillType').val() != WayBillType.WAYBILL_TYPE_FOC) {
		...
		if(response && !jQuery.isEmptyObject(response)) {
			...
			getTempStringForRates(response, quantity, chargeTypeId);
...
function checkChargesRates(obj) {
	...
	if(configuration.setChargeOnCCAttachedConfig == 'true'){
		if(!validateCCAttachedAmount(chargeMasterId)){
			return false;
		}
	}
	...
	if(configuration.editDeliveryAtBasedOnDlyChrges == 'true' && (chargeMasterId == BookingChargeConstant.DOOR_DELIVERY_BOOKING ||chargeMasterId == BookingChargeConstant.DOOR_DLY_OWN)) {
...
function calculateWeightAndQuantityWiseFreightAmount(chargeMasterId) {
	...
	if(quantityWiseChargeObj != null) {
		qtyAmnt			= getAmntFromQtyAmntAr(chargeMasterId, quantityWiseChargeObj);
	}
	
	if(weightWiseChargeAmount != null) {
		weightAmnt		= getAmntFromWeightAmntArr(chargeMasterId, weightWiseChargeAmount);
	}
...
function calculateLRLevelCharges(chargesRates, chargeMasterId, masterIdsArr) {
	...
	if(configuration.blockConvenienceChargeOnBookingScreen == 'true' || configuration.blockConvenienceChargeOnBookingScreen == true){
		//$('#charge' + BookingChargeConstant.CONVENIENCE).val(0);
		$('#charge' + BookingChargeConstant.CONVENIENCE).prop("readonly", "true");
	}else {
		$('#charge' + BookingChargeConstant.CONVENIENCE).removeAttr("readonly");
	}
	...
	if (chargesRates != null) {
		...
		if (chargeRate) {
			...
			if(configuration.AllowEnableDoorDelivery == 'true' && (chargeMasterId != configuration.doorDeliveryChargeId)) {
...
function applyRates() {
	...
	if($('#charge'+BookingChargeConstant.LR_CHARGE).val() <= 0 && configuration.LRCharge > 0)
		$('#charge'+BookingChargeConstant.LR_CHARGE).val(Number(configuration.LRCharge));
	...
	if(configuration.applyLoadingChargeOnFreightChargeAmt == 'true' || configuration.applyLoadingChargeOnFreightChargeAmt == true){
		...
		if(wayBillTypeId == WayBillType.WAYBILL_TYPE_PAID || wayBillTypeId == WayBillType.WAYBILL_TYPE_TO_PAY){
...
function calculateApplicableAmount(chargeId, chargeWiseAmount) {
	var qtyAmt		= 0;
	
	var rm					= applicableChargeRateObj[chargeId];
	var applicableOn		= rm.applicableOn;
	var rate				= Number(rm.rate);
		
	if(chargeWiseAmount[applicableOn] > 0)
		qtyAmt		= chargeWiseAmount[applicableOn] * rate / 100;
...
function getAmntFromQtyAmntAr(chargeMasterId, quantityWiseChargeObj) {
	var amount = 0;
	
	for (let [key, quantityWiseChargeAmount] of Object.entries(quantityWiseChargeObj)) {
		if($('#chargeType').val() == ChargeTypeConstant.CHARGETYPE_ID_QUANTITY && configuration.applyRatesOnlyOnceOnLRForArticleType == 'true') {
			if(quantityWiseChargeAmount[chargeMasterId] != null && quantityWiseChargeAmount[chargeMasterId]) {
				amount = parseFloat(amount) + parseFloat(quantityWiseChargeAmount[chargeMasterId]);
				break;
			}
		} else {
			if(quantityWiseChargeAmount[chargeMasterId] != null && quantityWiseChargeAmount[chargeMasterId]) {
				if(fixedChargeRateObj[chargeMasterId] != null && fixedChargeRateObj[chargeMasterId]) {
					amount = parseFloat(quantityWiseChargeAmount[chargeMasterId]);
					break;
				} else
					amount = parseFloat(amount) + parseFloat(quantityWiseChargeAmount[chargeMasterId]);
			}
		...
		if(configuration.allowConditionalRouteWiseCharges == 'true' && chargeMasterId == BookingChargeConstant.HAMALI) {
			var hamaliCharge = $("#chargedWeight").val() * 0.50;
			
			if(quantityWiseChargeAmount[chargeMasterId] != null && quantityWiseChargeAmount[chargeMasterId]) {
				if(hamaliCharge > quantityWiseChargeAmount[chargeMasterId]) {
					$("#charge" + chargeMasterId).val(hamaliCharge);
				} else {
					$("#charge" + chargeMasterId).val(quantityWiseChargeAmount[chargeMasterId]);
				}
			}
		}
	}
...
function getAmntFromWeightAmntArr(chargeMasterId, weightWiseChargeAmount) {
	var amount = 0;
	
	if(weightWiseChargeAmount[chargeMasterId] != null && weightWiseChargeAmount[chargeMasterId]) {
		amount = parseFloat(weightWiseChargeAmount[chargeMasterId]);
	}
	
	if(configuration.allowConditionalRouteWiseCharges == 'true') {
		var hamaliCharge = $("#chargedWeight").val() * 0.50;
		
		if(chargeMasterId == BookingChargeConstant.HAMALI
			&& weightWiseChargeAmount[chargeMasterId] != null && weightWiseChargeAmount[chargeMasterId]) {
			
			if(hamaliCharge > weightWiseChargeAmount[chargeMasterId]) {
				$("#charge" + chargeMasterId).val(hamaliCharge);
			} else{
				$("#charge" + chargeMasterId).val(weightWiseChargeAmount[chargeMasterId]);
			}
		}
	}
...
function getKiloMeterWiseRate() {
	...
	if(Number($("#chargeType").val()) != ChargeTypeConstant.CHARGETYPE_ID_KILO_METER) {
		return;
	}
	...
	if($('#wayBillType').val() != WayBillType.WAYBILL_TYPE_FOC) {
		var response	= getKMTypeRates(srcBranchId, destBranchId, vehicleTypeId, corporateAccountId, categoryTypeId, chargedWeight, rateTypeId, consigneeId, 0, 0, 0);
		if(response && !jQuery.isEmptyObject(response)) {
			weightFromDb		= 0;
			weightFromDb 		= getFreightChargeForRates(response);
			purefrieghtAmount	= getFreightChargeForRates(response);
			...
			if(weightFromDb > 0) {
				...
				if(weightTypeForRateApply == ChargeTypeConstant.WEIGHT_TYPE_ID_ACTUAL_WEIGHT){
					weightAmount	= parseFloat(weightFromDb * actualWeight * distance);
				} else{
					weightAmount	= parseFloat(weightFromDb * chargedWeight * distance);
				}
				$('#weightAmount').val(weightAmount.toFixed(2));
...
//applied various condition for weight type rates form rate master
function getKMTypeRates(srcBranchId, destBranchId, vehicleTypeId, corporateAccountId, categoryTypeId, userWeightInput, rateTypeId, consigneeId, packingTypeId, packingGrpTypeId, consignmentGoodsId) {
	var chargeTypeId		= ChargeTypeConstant.CHARGETYPE_ID_KILO_METER;
