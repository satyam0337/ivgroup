...
function printBillWindow() {
	...
	window.resizeTo(0,0);
	window.moveTo(0,0);
if(branchIdFoundForCrPrint){
	$("#tableContain").load( "/ivcargo/html/print/delivery/379_newDeliveryprint.html", function() {
		window.setTimeout(printAfterDelay, 500);
		autoChrg = getValueFromInputField('autoCharge');
		setPrintData();
		console.log(deliveryBranch)
	});
}else{
	$("#tableContain").load( "/ivcargo/html/print/delivery/379_deliveryprint.html", function() {
		window.setTimeout(printAfterDelay, 500);
		autoChrg = getValueFromInputField('autoCharge');
		setPrintData();
	...
	if(wayBillArray != null ) {
		...
		for(var i = 0; i < wayBillArray.length; i++) {
			...
			if(i < wayBillArray.length) {
				...
				LrNo	= createColumnInRow(tableRow, i, '', '', 'left', 'font-weight: bold', '');
...
function setCharges() {
	...
	for(var key in chargeIdWiseAmt){
		...
		if(key == ChargeTypeMaster.HAMALI_DELIVERY){
		...
		}else if (key == ChargeTypeMaster.DS_CHARGES) {
			 dsCharge2 = chargeIdWiseAmt[key];
			$('.DS_Charges').val(chargeIdWiseAmt[key]);
			...
			ddly = chargeIdWiseAmt[key];
			$('.DOOR_DELIVERY_DELIVERY').val(chargeIdWiseAmt[key]);
			...
			other = chargeIdWiseAmt[key];
			$('.OTHER_DELIVERY').val(chargeIdWiseAmt[key]);
		}
	}
