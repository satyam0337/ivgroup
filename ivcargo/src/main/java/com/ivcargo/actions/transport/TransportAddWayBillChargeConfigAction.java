...
public class TransportAddWayBillChargeConfigAction implements Action {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			ArrayList<CRTxn>						crTxnList						= null;	
			...
			if(crTxnList != null && crTxnList.size() > 0){
				crTxnArray = new CRTxn[crTxnList.size()];	
				crTxnList.toArray(crTxnArray);
			}
