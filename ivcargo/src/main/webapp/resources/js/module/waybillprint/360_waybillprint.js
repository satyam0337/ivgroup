...
function printBillWindow() {
	...
	$("#tableContain").load("/ivcargo/html/print/waybill/360_waybillprint.html", function() {
...
function setPrintData() {
	...
	$('.serviceType').append(consignmentSummary.serviceTypeName);
...
function setCharges() {
	if(wayBillCharges != null){
		...
		wayBillChrg	= wayBillCharges[BookingChargeConstant.EXPRESS_CHARGE];
		
		if (wayBillChrg != null) {
			$('.EXPRESS_CHARGE').append((wayBillChrg.chargeAmount).toFixed());
		} else	{
			$('.EXPRESS_CHARGE').append(0);
		}
