 ...
 */
var slickGridWrapper4 = null, wayBillId = 0, tripSheetId = 0, tripSheetStatus = 0, tripSheetNumber = 0, tripSheetRoute  = '';
define([
		...
		], function (Marionette, ElementModel, elementTemplateJs, ElementTemplate, TripHisabLSDetails, CreateNewVehicle, CreateNewDriver) {
	let _this, lsPropertyConfig, myNod = nod(), myNode, dispatchData, vehicleTypeId	= 0, tripSheet = false, doneTheStuff = false, validateTripSheet = false, isAnyDDDVWBs,
	TOKEN_VALUE	= "", TOKEN_KEY = "", languageKeyset, btVehicleModalConfirm,
	sourceBranch, lorryHireDetails, tripHisabLSInfo, tripHisabRequired = false, vehicleAgentMasterId = 0, totalLrCommission = 0,
	btDriverModalConfirm, isValidRC = true, didBindClockpicker = false;
	...
	return Marionette.ItemView.extend({
		initialize: function(data) {
			...
			if(lsPropertyConfig.showTransportModeFieldsOnFieldSelectionBasis)
				lsPropertyConfig.transportModeId = Number($("#transportModeSearchEle_primary_key").val());        
			else
				lsPropertyConfig.transportModeId = dispatchData[0].transportModeId;
			...
			languageKeyset			= data.Language;

			setTimeout(_this.executeFunctions, 200);
			
			if(lsPropertyConfig.isCrossingAgentPercentageCheckedByDefault)
					$('#isCommissionPercentageEle').prop('checked', true);
			else if(!lsPropertyConfig.showLsCommissionOnlyForCrossingAgent && lsPropertyConfig.allowLsCommission) {
				setTimeout(function() {
					$('#isCommissionPercentageEle').prop('checked', true);
			...
			if (lsPropertyConfig.showCommissionInPercentage) {		 
					 setTimeout(() => {  
					if (!$('#commissionInputEle').length) {
						$('<div class="commissionInputWrapper" style="display:inline-block;margin-left:5px">')
							.append('<input type="text" id="commissionInputEle" class="form-control commissionInputEle" style="width:90px;display:inline-block" placeholder="0">')
							.append('<span style="display:inline-block;margin-left:2px">%</span>')
							.insertBefore('#commissionCheckEle')
							.hide();
					}
					
					$('#commissionCheckEle').off('change').on('change', function() {
						const isChecked = $(this).is(':checked');
						$('.commissionInputWrapper').toggle(isChecked);
						if (isChecked) $('#commissionInputEle').focus();
						else $('#commissionInputEle, #commissionEle').val('');
					});
					
					$('#commissionInputEle').off('input').on('input', function() {
						let inputValue = this.value.replace(/[^0-9.]/g, '');
						let percentage = Math.min(parseFloat(inputValue) || 0, 100);
						...
						const totalFreight = dispatchData.reduce((sum, lr) => sum + (parseFloat(lr.freightAmount) || 0), 0);
						$('#commissionEle').val(((percentage / 100) * totalFreight).toFixed(2));
					});
					}, 1000);
			}
			...
			if(lsPropertyConfig.TruckSource) {
				setTimeout(() => {
					_this.setTruckSources(sourceBranch);
				}, 500);
			}

			if(lsPropertyConfig.validateLoaderName)
				addElementToCheckEmptyInNode(myNod, 'loaderNameEle', 'Please Enter Loader Name');
				
			if(lsPropertyConfig.isLoadingHamaliValidate)
				addElementToCheckNumericInNode(myNod, 'lsHamaliEle', 'Please Enter Valid Hamali/Labour Amount');
		
			if(lsPropertyConfig.hamalLeaderName) {
			...
			if(!lsPropertyConfig.TransportMode) {
				...
				if(lsPropertyConfig.isDriverName2Validate)
					addElementToCheckEmptyInNode(myNod,'driverName2Ele',cannotLeftBlankMsg)
				...
				if(lsPropertyConfig.allowLsCommission && $('#commissionEle').exists()) {
				...
				if(lsPropertyConfig.allowLsExpense && $('#lsExpenseEle').exists()) {
					$('#lsExpenseEle').val(0);
					addElementToCheckFloatInNode(myNod, 'lsExpenseEle', shouldBeNumericMsg)
				}
				
				const isAgentCrossingChecked = $("#isAgentCrossing").is(":checked");
				
				if(lsPropertyConfig.loaderNumber)
					addElementToCheckNumericInNode(myNod, 'loaderNumberEle', shouldBeNumericMsg);
				
				if(lsPropertyConfig.crossingHireAmount && isAgentCrossingChecked && $('#crossingHireEle').exists()) {
					$('#crossingHireEle').val(0);
					addElementToCheckNumericInNode(myNod, 'crossingHireEle', shouldBeNumericMsg);
				} else {
					$('#crossingHireEle').hide(); 
					$('#crossingHire').hide();
				}
			...
			if(lsPropertyConfig.dispatchRoute) {
				addAutocompleteElementInNode(myNod, 'dispatchRouteEle', 'Please Select Route!');
				
				let dispatchRouteAutoComplete = new Object();
			...
			if(lsPropertyConfig.vehicleType && !lsPropertyConfig.autoSelectVehicleTypeOnVehicleNumber) {
				...
				let vehicleTypeAutoComplete = new Object();
				...
				let autoDispatchRoute = $("#vehicleTypeEle").getInstance();
			...
			if(lsPropertyConfig.isAllowOnlyNumeric && manualLsSequenceCounterPresent != undefined && manualLsSequenceCounterPresent && lsPropertyConfig.isManualLSAllreadyChecked)
			...
			if(vehicleNumberMasterId == 0) {
				...
				if(!lsPropertyConfig.doNotShowSubInfoOfVehicleNumber) {
			...
			if(lsPropertyConfig.getDriverDetailsByMobileNumber) {
				$("#driverNameEle").prop('disabled', true);
				$("#driverName2Ele").prop('disabled', true);

				let show_fieldArray1	= [];
				let sub_asObject1		= {};
				
				show_fieldArray1.push('driverMasterId');
				show_fieldArray1.push('driverName');
				show_fieldArray1.push('mobileNumber');
				show_fieldArray1.push('licenceNumber');
				show_fieldArray1.push('driverId2Ele');
				
				sub_asObject1.driverName			= languageKeyset['driverName'];
				sub_asObject1.mobileNumber			= languageKeyset['driverNumber'];
				
				let autoDriverNumber = new Object();

				autoDriverNumber.url 				= WEB_SERVICE_URL + '/autoCompleteWS/getDriverDetailsOnMobileNumber.do';
				autoDriverNumber.primary_key 		= 'driverMasterId';
				autoDriverNumber.field 				= 'mobileNumber';
				autoDriverNumber.show_field 		= show_fieldArray1.join(', '); //do not remove driverMasterId from here
				
				if(!lsPropertyConfig.doNotShowSubInfoOfMobileNumber) {
					autoDriverNumber.sub_info 		= true;
					autoDriverNumber.sub_as 		= sub_asObject1;
				}
				
				autoDriverNumber.blurFunction		= _this.validateDriverMobNo;

				$("#driverMobileNumberEle").autocompleteCustom(autoDriverNumber);
				$("#driver2MobileNumberEle").autocompleteCustom(autoDriverNumber);
			}
			...
			autoLicenceNumber.show_field 	= 'driverMasterId, driverName, mobileNumber, licenceNumber, driverId2Ele'; //do not remove driverMasterId from here
			...
			$("#driverName2Ele").on('keyup', function(e) {
				_this.checkForSpecialCharacter(e,'driverName2Ele');
			});
			
			$("#driverName2Ele").on('keydown', function(e) {
				_this.checkForSpecialCharacter(e,'driverName2Ele');
			});
			...
			$(".ok").on('click', function() {
    			...
    			if(myNod.areAll('valid')) {
					...
					if(lsPropertyConfig.showDivisionSelection)
						jsonObject.divisionId		= dispatchData[0].divisionId;
										
					jsonObject.driverId2Ele = $('#driver2MobileNumberEle_primary_key').val();
					...
					jsonObject.isCrossingCheckBoxChecked = isCheckBoxChecked('crossingCheckBox');
					
					jsonObject.loadingHamali = $('#lsHamaliEle').val();

					finalJsonObj.jsondata	= JSON.stringify(jsonObject);

					jsonObject = new Object();
					$inputs = $('#lhpvChargeModal :input');
					...
					let isValidCrossingDest		= _this.checkValidCrossingDestination();
					let isManualLSInRange		= _this.checkManualLSInRange();
					...
					if(isValidCrossingDest && isManualLSInRange) {
						...
						if ($('#dispatchedDateEle').is(":visible") && !(_this.validateAutoLsDateWithAllLrs(dispatchData)))
							return false;
						
						if ($('#transportModeEle_primary_key').val() == TRANSPORTATION_MODE_AIR_ID && !validateArrivalAfterDispatch()) {
							// Prevent further action
							return;
						}
						
							if ($('#transportModeEle_primary_key').val() == TRANSPORTATION_MODE_RAIL_ID && !validateDispatchAfterArrival()) {
							return;
						}
						...
						if (lsPropertyConfig.isCommisionValidate) {
							const commissionVal = parseFloat($('#commissionEle').val()) || 0;
							const totalFreight = dispatchData.reduce((sum, lr) => sum + (parseFloat(lr.freightAmount) || 0), 0);

							if (commissionVal <= 0 && totalFreight>0) {
								showMessage('error', 'Please enter Commission Value before dispatch.');
								$('#commissionEle').focus();
								return false;
							}
						}
			...
			let autotransportModeName = $("#transportModeEle").getInstance();
			
			$(autotransportModeName).each(function() {
				this.option.source = transportationModeList;//coming from dispatch.js
			});
			
			let transportModeIdArray		= new Array();
			let uniqueTransportModeIdArray	= new Array();
			
			for(const element of dispatchData) {
				if(element.transportModeId > 0)
					transportModeIdArray.push(element.transportModeId);
			}
			...
			if(uniqueTransportModeIdArray.length > 1) {//transportationModeMap in dispatch.js
				...
				for(const tm of transportationModeList) {
			...
			if(!lsPropertyConfig.validateTransportMode) {
				if(Number($("#transportModeSearchEle_primary_key").val()) > 0) {
					$("#transportModeEle_primary_key").val(Number($("#transportModeSearchEle_primary_key").val()));
					$("#transportModeEle").val(transportationModeMap[Number($("#transportModeSearchEle_primary_key").val())]);
				} else {
					$("#transportModeEle_primary_key").val("");
					$("#transportModeEle").val("");
				}
			}
			...
			let specialChars	= (lsPropertyConfig.specialCharacterNotAllowed).split(",");
			...
			let returnType		= true;
			let specialChars	= (lsPropertyConfig.AllowedSpecialCharacters).split(",");

			let keynum 	= getKeyCode(evt);
			...
			if($('#transportModeEle_primary_key').val() == TRANSPORTATION_MODE_RAIL_ID || $('#transportModeEle_primary_key').val() == TRANSPORTATION_MODE_AIR_ID  || $('#transportModeEle_primary_key').val() == TRANSPORTATION_MODE_SHIP_ID) {
				myNod = nod();
				...
				if ($('#transportModeEle_primary_key').val() == TRANSPORTATION_MODE_AIR_ID && lsPropertyConfig.showAirlineDetails) {
					let flightNameAutoComplete = new Object();
					flightNameAutoComplete.url = WEB_SERVICE_URL + '/autoCompleteWS/getAllAirlineDetailsAutocomplete.do';
					flightNameAutoComplete.primary_key = 'airlineMasterId';
					flightNameAutoComplete.field = 'airlineMasterName';
					
					$("#flightNameEle").autocompleteCustom(flightNameAutoComplete);
					
					$('#dispatchedDateEle').on('change', function() {
						const dispatchDate = $(this).val(); // Format: yyyy-mm-dd

						if (dispatchDate) {
							$('#arrivalDateEle').attr('min', dispatchDate);
							
							if ($('#arrivalDateEle').val() < dispatchDate)
								$('#arrivalDateEle').val(dispatchDate);
						}
					});
					
					$("#arrivalTimeEle").addClass("clockpicker");
					...
					$("#dispatchedTimeEle").addClass("clockpicker");
					...
					// Set current time in input field
					$('.clockpicker').val(currentTime);
					
					$('#arrivalTimeEle, #arrivalDateEle').on('change', function() {
						const dispatchDate = $('#dispatchedDateEle').val();
						const dispatchTime = $('#dispatchedTimeEle').val();
						const arrivalDate = $('#arrivalDateEle').val();
						const arrivalTime = $('#arrivalTimeEle').val();

						if (dispatchDate && dispatchTime && arrivalDate && arrivalTime) {
							const dispatchDateTime = new Date(`${dispatchDate}T${dispatchTime}`);
							const arrivalDateTime = new Date(`${arrivalDate}T${arrivalTime}`);

							if (arrivalDateTime <= dispatchDateTime) {
								showMessage('error', 'Arrival time cannot be the same or earlier than dispatch time.');
								// Optionally reset to dispatch datetime
								$('#arrivalDateEle').val(dispatchDate);
								$('#arrivalTimeEle').val(dispatchTime);
							}
						}
					});

					addAutocompleteElementInNode1(myNod, 'flightNameEle', 'Select Flight Name');
					addAutocompleteElementInNode1(myNod, 'flightNumberEle', 'Enter Flight Number');
					addElementToCheckLength10InNode(myNod, 'flightNumberEle', 'Max Length Allowed be 10')
					addAutocompleteElementInNode1(myNod, 'arrivalDateEle', 'Select Arrival Date');
					addAutocompleteElementInNode1(myNod, 'dispatchedDateEle', 'Select Dispatch Date');
				}

				if ($('#transportModeEle_primary_key').val() == TRANSPORTATION_MODE_RAIL_ID && lsPropertyConfig.showRailDetails) {
					let trainNameAutoComplete = {};
					trainNameAutoComplete.url = WEB_SERVICE_URL + '/autoCompleteWS/getAllTrainDetailsAutocomplete.do';
					trainNameAutoComplete.primary_key = 'trainMasterId';
					trainNameAutoComplete.field = 'trainName';
					
					$("#trainNameEle").autocompleteCustom(trainNameAutoComplete);
					
					$('#departureDateEle').on('change', function() {
						const dispatchDate = $(this).val(); // Format: yyyy-mm-dd
					...
					$("#departureTimeEle").addClass("clockpicker");
					...
					$('#arrivalTimeEle, #arrivalDateEle').on('change', function() {
						const dispatchDate = $('#departureDateEle').val();
						const dispatchTime = $('#departureTimeEle').val();
						const arrivalDate = $('#arrivalDateEle').val();
						const arrivalTime = $('#arrivalTimeEle').val();
						...
						if (dispatchDate && dispatchTime && arrivalDate && arrivalTime) {
							...
							if (arrivalDateTime > dispatchDateTime) {
								showMessage('error', 'Dispatch Date/Time cannot be the same or earlier than Arrival time.');
					...
					addAutocompleteElementInNode1(myNod, 'trainNameEle', 'Select Train Name');
					addAutocompleteElementInNode1(myNod, 'arrivalDateEle', 'Select Arrival Date');
					addAutocompleteElementInNode1(myNod, 'departureDateEle', 'Select Departure Date');
				};
				
				if ($('#transportModeEle_primary_key').val() == TRANSPORTATION_MODE_SHIP_ID && lsPropertyConfig.showShipDetails) {
					var agentAutoComplete 			= new Object();
					agentAutoComplete.url 			= WEB_SERVICE_URL + '/autoCompleteWS/getVehicleAgentAutocomplete.do';
					agentAutoComplete.primary_key 	= 'vehicleAgentMasterId';
					agentAutoComplete.field 		= 'name';
					$("#shipAgentEle").autocompleteCustom(agentAutoComplete);
					
					addAutocompleteElementInNode1(myNod, 'vesselNameEle', 'Select Vessel Name');
					addAutocompleteElementInNode1(myNod, 'shipAgentEle', 'Select Agent Name');
				};

				$('div #region').removeClass('show').addClass('hide');
			} else {
				_this.validateFeilds();
			...
			if(dispatchData != undefined && dispatchData.length > 0) {
				if(lsPropertyConfig.allowCrossingBranchWiseData){
					 let firstWayBillBranchId = dispatchData[0]["wayBillDestinationBranchId"];
	   				 let isSameBranchId = dispatchData.every(element => element["wayBillDestinationBranchId"] === firstWayBillBranchId);
	
					for(const element of dispatchData) {
						destinationBranches.push(element["crossingBranchId"])
						
						if(isSameBranchId)
							destinationBranches.push(element["wayBillDestinationBranchId"]);
					}
				} else {
					for(const element of dispatchData) {
						destinationBranches.push(element["wayBillDestinationBranchId"]);
			...
			let autoDest = new Object();

			if($("#crossingBranchEle_primary_key").val() > 0) {
				let unloadingCrossingBranchObj	= new Object()
				...
				let unloadingCrossingBranchArr = new Array();
			...
			if(manualLsSequenceCounterPresent != undefined && manualLsSequenceCounterPresent && lsPropertyConfig.RangeCheckInManualLS) {
				let minRange			= manualLSSequenceCounter.minRange;
				let maxRange			= manualLSSequenceCounter.maxRange;
			...
			let isLhpvCreation	= response.isLhpvCreation;
			
			let MyRouter = new Marionette.AppRouter({});
			
			if(isLhpvCreation != undefined && isLhpvCreation)
				MyRouter.navigate('&modulename=loadingSheetNew&'+MASTERID+"="+response.dispatchLedgerId+'&'+MASTERID2+"="+response.lsNumber+ '&isLhpvCreation='+isLhpvCreation, {trigger: true});
			else
				MyRouter.navigate('&modulename=loadingSheetNew&'+MASTERID+"="+response.dispatchLedgerId+'&'+MASTERID2+"="+response.lsNumber, {trigger: true});
		...
		}, setDriverDetailByMobNumber : function(response) {
			let obj	= response.DriverMaster;

			if(obj != undefined) {
				$("#driverNameEle").val(obj.driverName);
				$("#driverMobileNumberEle_primary_key").val(obj.driverMasterId);
				$("#driverIdEle").val(obj.driverMasterId);
			}
		}, setDriverDetailByMobNumber2 : function(response) {
			let obj	= response.DriverMaster;

			if(obj != undefined) {
				$("#driverName2Ele").val(obj.driverName);
				$("driver2MobileNumberEle_primary_key").val(obj.driverMasterId);
				$("#driverId2Ele").val(obj.driverMasterId);
			}
		}, checkForNewDriver : function() {
			let driverValue 		= $('#'+$(this).attr('id')+'_primary_key').val();
			let driverValueString	= $("#driverLicenceNumberEle").val();
		...
		}, validateDriverMobNo : function() {
			let driverValue 		= $('#' + $(this).attr('id') + '_primary_key').val();
			let driverValueString	= $("#driverMobileNumberEle").val();
			let mobileNoValueString	= $("#driver2MobileNumberEle").val();
			
			if (driverValueString === mobileNoValueString) {
				showAlertMessage('error', "Please Select Different MobileNo.");
				$("#driver2MobileNumberEle").val('');
				return;  
			}

			if (driverValue == "" && driverValueString.length > 0) {
				addAutocompleteElementInNode(myNod, 'driverMobileNumberEle', 'Select Proper Driver Number');
				
				if(mobileNoValueString != "" && ($("driver2MobileNumberEle_primary_key").val() == undefined || $("driver2MobileNumberEle_primary_key").val() == 0))
					addAutocompleteElementInNode(myNod, 'driver2MobileNumberEle', 'Select Proper Driver Number 2');
				else
					myNod.remove('driver2MobileNumberEle');
			} else if (lsPropertyConfig.Driver2MobileNumber && $(this).attr('id') == 'driver2MobileNumberEle' && mobileNoValueString != undefined && mobileNoValueString.length == 10) {
				let jsonObject	= {};
				jsonObject.MobileNumber		= mobileNoValueString;
				getJSON(jsonObject, WEB_SERVICE_URL	+ '/driverWS/getDriverDetailsByMobileNumber.do', _this.setDriverDetailByMobNumber2, EXECUTE_WITHOUT_ERROR);
			} else {
				let jsonObject	= {};
				jsonObject.MobileNumber		= driverValueString;
				getJSON(jsonObject, WEB_SERVICE_URL	+ '/driverWS/getDriverDetailsByMobileNumber.do', _this.setDriverDetailByMobNumber, EXECUTE_WITHOUT_ERROR);
			}
			...
			jsonObject['validateTripSheet']				= lsPropertyConfig.validateTripSheetFromFleetop;
			...
			let incomeExpenseChargeMaster	= response.IncomeExpenseChargeMaster;
				...
				btModalConfirm.on('ok', function() {
					...
					btModal.on('ok', function() {
						...
						if(myNode.areAll('valid')) {
							let jsonObject 	= new Object();
							let $inputs 	= $('#tripLSDetails :input');
							//this will iterate all elements and will set the name as key and value of Element as value in jsonObject
			...
			if(lsPropertyConfig.validateTripSheetFromFleetop)
			...
			if(response.driverMasterList.length == 1) {
				let driverAutoComplete = new Object();
				...
				$("#driverIdEle").val("");
				$("#driverLicenceNumberEle").val("");
				$("#driverLicenceNumberEle_primary_key").val("");
			...
			setTimeout(function(){
				let vehicleValue 		= $('#'+ele+'_primary_key').val();
				let vehicleValueString	= $("#vehicleNumberEle").val();
				let newVehicle 			= $("#vehicleNumberEle").val();
			...
			let autocompleteSourceAndDest = $('#vehicleNumberEle').getInstance();
			...
			let crossingAgentId		= $('#crossingAgentEle_primary_key').val();
			let truckDestination	= $('#truckDestinationEle_primary_key').val();
			let crossingBranchIds	= $('#agentDestinationAreaEle').val();
			
			if(crossingAgentId > 0 && crossingBranchIds != undefined) {
				let crossingBranchIdArr	= crossingBranchIds.split(',');
				
				if(!isValueExistInArray(crossingBranchIdArr, truckDestination)) {
			...
			if(manualLsSequenceCounterPresent != undefined && manualLsSequenceCounterPresent && lsPropertyConfig.RangeCheckInManualLS) {
				if ($('#isManualDispatch').is(':checked')) {
					...
					let manualLSNumber	= parseInt($('#manualLSNumber').val());
					let minRange 		= parseInt($('#minRangeSequenceEle').val());
					let maxRange 		= parseInt($('#maxRangeSequenceEle').val());
			...
			for(const element of dispatchData){
				...
				let manualLSDate = $('#manualLSDate').val().split("-");
				manualLSDate = new Date(manualLSDate[2], parseInt(manualLSDate[1],10) - 1, manualLSDate[0]);
				
				if(waybillDate > manualLSDate) {
					showMessage('error','Manual LS Date earlier then Booking Date of LR Number ' + element.wayBillNumber);
					return false;
				}
			}
			...
			let jsonValue 	= $('#vehicleNumberEle').attr('sub_info');
			let obj 		= eval( '(' + jsonValue + ')' );
			...
			if(partBCheckBox != null && $("#manualLSDate").val() != maxDate) {
				$(".checkBoxClass").switchClass("hide", "show");
				partBCheckBox.checked = true;
			} else
				$(".checkBoxClass").switchClass("show", "hide");
		}, validateAutoLsDateWithAllLrs: function(dispatchData) {
			const dispatchedRaw = $('#dispatchedDateEle').val();

			if (!dispatchedRaw || dispatchedRaw.split("-").length !== 3) {
				showMessage('error', 'Invalid or missing dispatched date');
				return false;
			}

			// Parse as yyyy-mm-dd
			const [yyyy, mm, dd] = dispatchedRaw.split("-");
			const dispatchedDate = new Date(yyyy, parseInt(mm, 10) - 1, dd);

			for (const element of dispatchData) {
				let waybillDateStr = element.incomingDateTimeStampString;

				const [dd2, mm2, yyyy2] = waybillDateStr.split("-");
				const waybillDate = new Date(yyyy2, parseInt(mm2, 10) - 1, dd2);

				if (dispatchedDate < waybillDate) {
					showMessage('error', 'Dispatched Date cannot be earlier than Booking Date of LR Number ' + element.wayBillNumber);
					return false;
				}
			}
...
function setValue(cellValue,selectedDeliveryTo,cellId) {
	let slickData 			= slickGridWrapper4.getItems();
...
function validateArrivalAfterDispatch() {
	const dispatchDate = $('#dispatchedDateEle').val();
	const dispatchTime = $('#dispatchedTimeEle').val();
	const arrivalDate  = $('#arrivalDateEle').val();
	const arrivalTime  = $('#arrivalTimeEle').val();
...
function validateDispatchAfterArrival() {
	const dispatchDate = $('#departureDateEle').val();
	const dispatchTime = $('#departureTimeEle').val();
	const arrivalDate = $('#arrivalDateEle').val();
	const arrivalTime = $('#arrivalTimeEle').val();
