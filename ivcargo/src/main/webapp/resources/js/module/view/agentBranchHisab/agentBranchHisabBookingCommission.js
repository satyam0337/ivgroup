define([
	...
	,PROJECT_IVUIRESOURCES + '/resources/js/module/view/agentBranchHisab/agentbranchhisabbookingcommissionfilepath.js'
	,'jquerylingua'
	,'language'
	,'slickGridWrapper2'
	...
	return Marionette.LayoutView.extend({
		initialize : function(jsonObjectData) {
			...
			getJSON(jsonObject, WEB_SERVICE_URL + '/agentBranchHisabWS/getAgentBranchHisabBookingCommissionDetails.do?', _this.renderAgentBranchHisabTopayBookingElements, EXECUTE_WITH_ERROR);
			return _this;
			...
			setTimeout(function(){
				$("#modalBody").load("/ivcargo/html/module/AgentBranchHisab/agentBranchHisabBookingCommission.html",function() {
					baseHtml.resolve();
				});
			},200);
			
			if(response.message != undefined){
				refreshAndHidePartOfPage('bookingCommissionDetailsDiv', 'hide');
				hideLayer();
			...
			$.when.apply($, loadelement).done(function(){
				...
				if(response.tableConfig.CorporateAccount != undefined) {
					$('#bookingCommissionDetailsDiv').show();
					hideAllMessages();
