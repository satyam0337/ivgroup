/*	AutoComplete Feature withou AJAX Call */

var stringId =0;  //Id of the text inserted in textBox
...
function actb(obj,ca){
	...
	/* --- Styles --- */
	this.actb_bgColor = '#999999';
	this.actb_textColor = '#000000';
	this.actb_hColor = '#FFFFFF';
	...
	function actb_generate(){
		...
		for (i=0;i<actb_self.actb_keywords.length;i++){
			if (actb_bool[i]){
				...
				c.innerHTML = actb_parse(actb_self.actb_keywords[i][2]);
				c.id = 'tat_td'+(j);
				if(first){
	...
	function actb_penter(){
		...
		for (var i=0;i<=actb_self.actb_keywords.length;i++){
			if (actb_bool[i]) c++;
			if (c == actb_pos){
				word = actb_self.actb_keywords[i][2];
				stringId=actb_self.actb_keywords[i][1];
				break;
			}
		}
	...
	function actb_tocomplete(kc){
		...
		for (i=0;i<actb_self.actb_keywords.length;i++){
			actb_bool[i] = false;
			if (re.test(actb_self.actb_keywords[i][2])){
				actb_total++;
				actb_bool[i] = true;
				actb_kwcount++;
...
function populateStateByCountryId(coutryId, destination) {
	var jsonObject			= new Object();

	jsonObject["CountryId"]	= coutryId;
	
	showLayer();

	$.ajax({
		type		: "POST",
		url			: WEB_SERVICE_URL+'/selectOptionsWS/getStateByCountryId.do',
		data		: jsonObject,
		dataType	: 'json',
		success: function(data) {
			...
			if(typeof stateList == 'undefined' || stateList.length == 0) {
				showMessage('info', 'Country Not Found');
			}
			
			$('#' + destination).find('option').remove().end();
			$('#' + destination).append('<option value="0">---- Select  State ----</option>');
			
			if(stateList != null && typeof stateList !== 'undefined') {
				for(var i = 0; i < stateList.length; i++) {
					$('#' + destination).append('<option value="'+ stateList[i].stateId + '">' + stateList[i].stateName + '</option>');
				} 
			}
...
function setSaidToContainAutocomplete(obj) {
	...
	$("#" + obj.id).autocomplete({
		source: function (request, response) {
			...
			console.log(e);
			$('#consignmentGoodsId_' + i).val(u.item.id);
		},
		minLength: 2,
		delay: 100
	});
...
function setPackingTypeAutocomplete(obj) {
	var i = (obj.id).split('_')[1];
		...
		source: function (request, response) {
			$.getJSON(WEB_SERVICE_URL + '/autoCompleteWS/getPackingTypeByNameAndGroupId.do?term=' + request.term, function (data) {
				...
				return {
					...
					id			: item.packingTypeMasterId
				};
			 }));
			});
		}, select: function (e, u) {
			console.log(e);
			$('#packingTypeId_' + i).val(u.item.id);
		},
		minLength: 2,
		delay: 100
	});
...
function getCitiesByStateId(stateId, cityEle) {
	var jsonObject				= new Object();
	...
	$.ajax({
		...
		success: function(data) {
			...
			if(typeof selectedCityId != 'undefined' && selectedCityId > 0) {
				$('#' + cityEle).val(selectedCityId);
			}
...
function getExecutiveByBranchId(branchId, nameEle) {
	var jsonObject				= new Object();

	jsonObject["sourceBranchId"]		= branchId;
	jsonObject["displaySuperAdmin"]		= displaySuperAdmin || false;
	jsonObject["displayOnlyActiveUsers"]= displayOnlyActiveUsers || false;
	jsonObject["AllOptionsForExecutive"]= AllOptionsForExecutive || false;
	...
	$.ajax({
		...
		dataType	: 'json',
		success: function(data) {
			var executiveList		= data.executiveList;
			
			operationOnSelectTag(nameEle, 'removeAll', '', ''); //function calling from genericfunction.js
			operationOnSelectTag(nameEle, 'addNew', '---- Select  Executive ----', 0); //function calling from genericfunction.js
			
			if(executiveList != null && typeof executiveList !== 'undefined') {
				for(var i = 0; i < executiveList.length; i++) {
					operationOnSelectTag(nameEle, 'addNew', executiveList[i].executiveName, executiveList[i].executiveId);
				} 
			}
