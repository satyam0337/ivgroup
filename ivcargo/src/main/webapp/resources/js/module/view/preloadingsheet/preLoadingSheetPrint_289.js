define(['elementTemplateJs'
	...
	return ({
		renderElements : function(response){
			...
			require(['text!/ivcargo/template/preloading/preloadingsheet_14.html'],function(PreLoadingSheet){
				...
				var totalPaidFreight = 0;
				var totalTopayFreight = 0;
				var totalLoading = 0;
				for(var i = 0;i<finalJsonObj.length;i++){
					...
					if(pendingDispatchModel.pendingQuantity > 0){
						...
						totalPaidFreight += pendingDispatchModel.paidFreightTotal;
						totalTopayFreight += pendingDispatchModel.topayFreightTotal;
						totalLoading += pendingDispatchModel.wayBillLoadingCharge;
						totalNoOfLR++;
					}
				}
				...
				$('.totalPaidFreight').last().html(totalPaidFreight);
				$('.totalTopayFreight').last().html(totalTopayFreight);
				$('.totalLoading').last().html(totalLoading);
