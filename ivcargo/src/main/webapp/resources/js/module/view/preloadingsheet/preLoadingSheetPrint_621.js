define(['elementTemplateJs'
	...
	return ({
		renderElements : function(response){
			...
			this.setDataForView(finalJsonObj,response);
		},setDataForView:function(finalJsonObj,response){
			
			require(['text!/ivcargo/template/preloading/preloadingsheet_621.html'],function(PreLoadingSheet){
				...
				for(var i = 0;i<finalJsonObj.length;i++){
					...
					if(pendingDispatchModel.pendingQuantity > 0){
						...
						totalDeclaredValue+= pendingDispatchModel.declaredValue;
						totalNoOfLR++;
					}
				...
				for(var key in chunkArray){
					...
					_this.renderTableValues(chunkArray[key],'.leftdivTableBody',response);
				...
				$('.totalDeclaredValue').html(totalDeclaredValue);

				
				_this.setHeaderData(headerCollection);
				_this.setTableRowData(finalJsonObj,allTypeChargedWeightTotal,allTypeActualWeightTotal,totalNoOfQuantity,bookingTotalToPay,allTypeBookingTotal,noOfLRPaid,noOfLRToPay,noOfLRTBB,totalDeclaredValue);
				hideLayer();	
				_this.showPrintOnPopup();
			})
			
		},setTableRowData:function(tableData,allTypeChargedWeightTotal,allTypeActualWeightTotal,totalNoOfQuantity,bookingTotalToPay,allTypeBookingTotal,noOfLRPaid,noOfLRToPay,noOfLRTBB,totalDeclaredValue){
			hideLayer();
			$('#mainTable').empty();
			    ...
			    let headerColumnArray = [
			        ...
			        '<th style="text-align: center;font-size:13px;  border-right: 1px solid black; border-bottom: 1px solid black; ">LR<br>No</th>',
			        '<th style="text-align: center;font-size:13px;  border-right: 1px solid black; border-bottom: 1px solid black;">PARTICULAR</th>',
			        '<th style="text-align: center;font-size:13px;  border-right: 1px solid black; border-bottom: 1px solid black;">INVOICE NO</th>',
			        '<th style="text-align: center;font-size:13px;  border-right: 1px solid black; border-bottom: 1px solid black; ">WEI<br>HT</th>',
			        '<th style="text-align: center;font-size:13px;  border-right: 1px solid black; border-bottom: 1px solid black">VOG</th>',
			        '<th style="text-align: center;font-size:13px;  border-right: 1px solid black; border-bottom: 1px solid black; ">AMOUNT</th>',
			        '<th style="text-align: center;font-size:13px;  border-right: 1px solid black; border-bottom: 1px solid black;">QTY</th>',
			        '<th style="text-align: center;font-size:13px;  border-right: 1px solid black; border-bottom: 1px solid black;">REMARKS</th>',			        
			    ];
          ...
          for (var i = 1; i < tableData.length; i++) {
				...
				dataColumnArray.push("<td style='text-align: center; border-right: 1px solid black; border-bottom: 1px solid black;'>"+ el.packingTypeMasterName+" "+el.consignmentSummarySaidToContain +"</td>");
				dataColumnArray.push("<td style='text-align: center; border-right: 1px solid black; border-bottom: 1px solid black; word-wrap:break-word; white-space: normal;'>"+ el.consignmentSummaryInvoiceNumber +"</td>");
			    ...
			    dataColumnArray.push("<td style='text-align: center;  border-bottom: 1px solid black; word-wrap:break-word; white-space: normal;'>"+ el.wayBillRemark +"</td>");
		    ...
		    for (let i=1; i <=headerColumnArray.length; i++) {
				if(i==6){
			      ...
			      totalDataArray.push("<td style='text-align: center; border-left: 1px solid black; border-right: 1px solid black; font-weight: bold;'>" + totalNoOfQuantity + "</td>");
					...
					totalDataArray.push("<td style='text-align: center; border-left: 1px solid black; border-right: 1px solid black; font-weight: bold;'>" + allTypeChargedWeightTotal + "</td>");
				}else if(i==5){
					totalDataArray.push("<td style='text-align: center; border-left: 1px solid black; border-right: 1px solid black; font-weight: bold;'>" + totalDeclaredValue + "</td>");
				}
				else{
		...
		},renderTableValues:function(chunkArray,className,response){
			var htmlVariable = '';
			...
			setCompanyLogos(response.PrintHeaderModel.accountGroupId);
			...
			childwin = window.open('', 'newwindow', config = 'height=' + height + ',width=' + width + ',left=' + left + ',top=' + top + ', toolbar=no, menubar=no, scrollbars=no, resizable=no,location=no, directories=no, status=no');
			 var clonedElement = $("#myGrid").clone(true);
