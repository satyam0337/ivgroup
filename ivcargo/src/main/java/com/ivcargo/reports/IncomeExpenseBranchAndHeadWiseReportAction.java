...
import com.businesslogic.IncomeExpenseBranchAndHeadWiseBLL;
...
public class IncomeExpenseBranchAndHeadWiseReportAction implements Action {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			new InitializeIncomeExpenseBranchAndHeadWiseReportAction().execute(request, response);
			...
			if(request.getParameter("headid") != null){
				...
				final var branchWiseIncomeExpenseReportDetailsProperties = ReadAllConfigurationsBllImpl.getInstance().getReportConfigurationData(ReportIdentifierConstant.INCOME_EXPENSE_BRANCH_AND_HEADWISE_REPORT, executive.getAccountGroupId());
				...
				final var	valueoutObject =  IncomeExpenseBranchAndHeadWiseBLL.getInstance().getIncomeExpenseBranchAndHeadWiseReport(valueInobject);
				...
				}else if(valueoutObject != null && valueoutObject.get("headWiseHM") != null && valueoutObject.get("branchWiseHM")!= null) {
					request.setAttribute("headWiseHM", valueoutObject.get("headWiseHM"));
					request.setAttribute("branchWiseHM", valueoutObject.get("branchWiseHM"));
					request.setAttribute("finalAmountTotal", valueoutObject.get("finalAmountTotal"));
					request.setAttribute("headWiseAmntGrandTotal", valueoutObject.get("headWiseAmntGrandTotal"));
					request.setAttribute("branchWiseAmntGrandTotal", valueoutObject.get("branchWiseAmntGrandTotal"));
					request.setAttribute("mainHeadsTrMap", valueoutObject.get("mainHeadsTrMap"));
					request.setAttribute("headIdsWithZero", valueoutObject.get("headIdsWithZero"));
					request.setAttribute("zeroColumnArrList", valueoutObject.get("zeroColumnArrList"));
				} else {
					error.put("errorCode", CargoErrorList.REPORT_NOTFOUND);
					error.put("errorDescription", CargoErrorList.REPORT_ERROR_DESCRIPTION);
					request.setAttribute("cargoError", error);
