	...
	return ({
		setDataForView : function (responseObj){
			...
			if(printType != undefined){
				//if(responseObj.FlavorConfiguration.dispatchLSPrintFlavor == 'loadingSheet_91') {
					$("#mainContent").load("/ivcargo/html/print/dispatch/LoadingSheet_91.html", function() {
						baseHtml.resolve();
					});
				//}
			}
			...
			$.when.apply($, loadelement).done(function() {
				...
				$(".lsdetails").addClass('hide');
				$('[data-footerdetails="footerInfo"]').addClass('hide');
				...
				for(var key in accountGroupKey){
					_this.setHeaderWiseData(key,accountGroupKey.length,groupDataObject[accountGroupKey[key]],dispatchDataAccountGroupWise[accountGroupKey[key]],dispatchDataAccountGroupWise[accountGroupKey[key]],dispatchLSLRCharge,dispatchLSHeaderAccountGroupWise[accountGroupKey[key]],dispatchLSLRFormSummaryAccountGroupWise[accountGroupKey[key]],showPartialLSDetailsSummary,displayData,lhpvChargeList,printType);
				}
				hideLayer();
					setTimeout(function(){
					window.print();
		...
		}, setHeaderWiseData : function (iteration,totalGroup,groupData,contentCollection,dispatchDataAccountGroupWise,dispatchLSLRCharge,dispatchLSHeaderAccountGroupWise,dispatchLSLRFormSummaryAccountGroupWise,showPartialLSDetailsSummary,displayData,lhpvChargeList,printType){
			totalRow						= 0;
			headerPrint						= true;
			var groupHeaderWiseTable		= $(".showaptlssummary");
				...
				for(var i = 0; i < columnObjectForDetails.length ; i++){
					...
					if(datapiker == 'name'){
						if(typeof lsSrcBranch !== 'undefined' && lsSrcBranch != undefined 
							...
							} else if(lsSrcBranch.branchId == 16854){
								newtd.append('Bato Cargo Movers');
							}  else {
