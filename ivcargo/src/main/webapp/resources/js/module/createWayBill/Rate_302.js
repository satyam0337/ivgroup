...
function getChargesRates() {
	...
	operationalBranchId		= $('#sourceBranchId').val();
	...
	if (configuration.CalculateCarrierRiskChargeOnDeclaredValue == 'true'){
		configObject = filterLRLevelRates(4, branchId, destBranchId, '', '', '', corporateAccountId, '', '', '',ChargeConfiguration.APPLICABLE_ON_CATEGORY_ID_FIELD, ChargeConfiguration.FIELD_ID_DECLARED_VALUE, declaredValue, 0);
		checkAndInsertRates(configObject, configData, 0, 3);

		if(configuration.applyOnlyPartyWiseLRLrLevelCharge == 'false') {
			configObject = filterLRLevelRates(4, branchId, destBranchId, '', '', '', 0, '', '', '',ChargeConfiguration.APPLICABLE_ON_CATEGORY_ID_FIELD, ChargeConfiguration.FIELD_ID_DECLARED_VALUE, declaredValue ,0);
			checkAndInsertRates(configObject, configData, 0, 3);
		}
	}
	configObject = filterLRLevelRates(3, branchId, destBranchId, '', '', '', corporateAccountId, '', '', '', 0, 0, 0, operationalBranchId);
	checkAndInsertRates(configObject, configData, 0, 3);
	
	if(configuration.applyOnlyPartyWiseLRLrLevelCharge == 'false') {
		if(configObject == null || configObject.length == 0) {
			configObject = filterLRLevelRates(3, branchId, destBranchId, '', '', '', 0, '', '', '', 0, 0, 0, operationalBranchId);
			checkAndInsertRates(configObject, configData, 0, 3);
		}
	}
...
function filterRates(srcBranchId, destBranchId, categoryTypeId, vehicleTypeId, packingTypeId, corporateAccountId, chargeTypeId, rateTypeId, chargeSectionId, packingGrpTypeId, consignmentGoodsId, consigneeId, operationalBranchId) {
	...
	if(!jQuery.isEmptyObject(wayBillRates)) {
		for (var i = 0; i < wayBillRates.length; i++) {
			...
			if(Number(datarates.accountGroupId) 				== Number(accountGroupId)
					&& (Number(datarates.branchId) 				== Number(srcBranchId) || Number(datarates.branchId) == Number(operationalBranchId))
...
//filter rate as par user selection get revert filterd data 
function filterLRLevelRates(filter, srcBranchId, destBranchId, categoryTypeId, vehicleTypeId, packingTypeId, corporateAccountId, chargeTypeId, rateTypeId, chargeSectionId, applicableOnCatetgoryId, fieldId, declaredValue, operationalBranchId) {
	var accountGroupId	= executive.accountGroupId;
	var arr 			= new Array();
		...
		if(!jQuery.isEmptyObject(chargesConfigRates) && chargesConfigRates.length > 0) {
			for (var i = 0; i < chargesConfigRates.length; i++) {
				...
				if(Number(rates.accountGroupId) 			 == Number(accountGroupId)
						&& (Number(rates.branchId) 			 == Number(srcBranchId) || Number(rates.branchId) == Number(operationalBranchId))
				...
				if(rateId_mast[rateMaster[i].chargeTypeMasterId] == null || (typeof rateId_mast[rateMaster[i].chargeTypeMasterId] == 'undefined')) {
					if(rateMaster[i].minWeight > 0 && rateMaster[i].maxWeight > 0) {
						if(configuration.isApplyFixRateonArticleRate == 'true' || configuration.isApplyFixRateonArticleRate == true) {
							if(rateMaster[i].minWeight == rateMaster[i].maxWeight) {
					...
					if(rateMaster[i].minWeight > 0 && rateMaster[i].maxWeight > 0) {
						if(configuration.isApplyFixRateonArticleRate == 'true' || configuration.isApplyFixRateonArticleRate == true) {
							if(rateMaster[i].rate > rateId_mast[rateMaster[i].chargeTypeMasterId]) {
								if(rateMaster[i].minWeight == rateMaster[i].maxWeight) {
									rateId_mast[rateMaster[i].chargeTypeMasterId] = rateMaster[i].rate;
									actualSlabValue = rateMaster[i].minWeight;
									isApplyFixRateonArticleRateMinMaxArticleWise = true;
...
function getWghtTypeRates(srcBranchId, destBranchId, vehicleTypeId, corporateAccountId, categoryTypeId, packingTypeId, packingGrpTypeId, userWeightInput, rateTypeId, consignmentGoodsId, consigneeId, operationalBranchId) {
	...
	//Route level charges
	rateMaster = filterRates(srcBranchId, destBranchId, categoryTypeId, vehicleTypeId, packingTypeId, corporateAccountId, chargeTypeId, rateTypeId, chargeSectionId, packingGrpTypeId, consignmentGoodsId, consigneeId, operationalBranchId);
	checkAndInsertRates(rateMaster, rateId_mast, userWeightInput, 1);
	
	rateMaster = filterRates(srcBranchId, destBranchId, categoryTypeId, vehicleTypeId, packingTypeId, corporateAccountId, chargeTypeId, rateTypeId, chargeSectionId, packingGrpTypeId, 0, consigneeId, operationalBranchId);
	checkAndInsertRates(rateMaster, rateId_mast, userWeightInput, 1);
	
	rateMaster = filterRates(srcBranchId, destBranchId, categoryTypeId, vehicleTypeId, 0, corporateAccountId, chargeTypeId, rateTypeId, chargeSectionId, packingGrpTypeId, 0, consigneeId, operationalBranchId);
	checkAndInsertRates(rateMaster, rateId_mast, userWeightInput, 1);
	
	rateMaster = filterRates(srcBranchId, destBranchId, categoryTypeId, vehicleTypeId, packingTypeId, corporateAccountId, chargeTypeId, rateTypeId, chargeSectionId, packingGrpTypeId, consignmentGoodsId, 0, operationalBranchId);
	checkAndInsertRates(rateMaster, rateId_mast, userWeightInput, 1);
	
	rateMaster = filterRates(srcBranchId, destBranchId, categoryTypeId, vehicleTypeId, packingTypeId, corporateAccountId, chargeTypeId, rateTypeId, chargeSectionId, packingGrpTypeId, 0, 0, operationalBranchId);
	checkAndInsertRates(rateMaster, rateId_mast, userWeightInput, 1);
	
	rateMaster = filterRates(srcBranchId, destBranchId, categoryTypeId, vehicleTypeId, 0, corporateAccountId, chargeTypeId, rateTypeId, chargeSectionId, packingGrpTypeId, 0, 0, operationalBranchId);
	checkAndInsertRates(rateMaster, rateId_mast, userWeightInput, 1);
	
	//Loading type charges
	rateMaster = filterRates(srcBranchId, 0, categoryTypeId, 0, 0, corporateAccountId, chargeTypeId, rateTypeId, chargeSectionId, packingGrpTypeId, 0, 0, operationalBranchId);
	checkAndInsertRates(rateMaster, rateId_mast, userWeightInput, 1);
	...
	//Fixed Party charges
	rateMaster = filterRates(srcBranchId, 0, categoryTypeId, 0, 0, corporateAccountId, 0, rateTypeId, 8, 0, 0, 0, operationalBranchId);
	checkAndInsertRates(rateMaster, rateId_mast, userWeightInput, 2);
	
	//Filter Slab Wise Rate
	rateMaster = filterSlabRates(srcBranchId, destBranchId, categoryTypeId, vehicleTypeId, 0, corporateAccountId, chargeTypeId, rateTypeId, 0, 0, 0, consigneeId, operationalBranchId);
	checkAndInsertRates(rateMaster, rateId_mast, userWeightInput, 1);
	
	//Filter Slab Wise Rate
	rateMaster = filterSlabRates(srcBranchId, destBranchId, categoryTypeId, vehicleTypeId, 0, corporateAccountId, chargeTypeId, rateTypeId, 0, 0, 0, 0, operationalBranchId);
	checkAndInsertRates(rateMaster, rateId_mast, userWeightInput, 1);
	...
	if(categoryTypeId == RateMaster.CATEGORY_TYPE_PARTY_ID ) {
		...
		//Route level charges
		rateMaster = filterRates(srcBranchId, destBranchId, RateMaster.CATEGORY_TYPE_GENERAL_ID, vehicleTypeId, packingTypeId, 0, chargeTypeId, 0, chargeSectionId, packingGrpTypeId, consignmentGoodsId, consigneeId ,operationalBranchId);
		checkAndInsertRates(rateMaster, rateId_mast, userWeightInput ,1);
		
		rateMaster = filterRates(srcBranchId, destBranchId, RateMaster.CATEGORY_TYPE_GENERAL_ID, vehicleTypeId, packingTypeId, 0, chargeTypeId, 0, chargeSectionId, packingGrpTypeId, 0, consigneeId, operationalBranchId);
		checkAndInsertRates(rateMaster, rateId_mast, userWeightInput ,1);
		
		rateMaster = filterRates(srcBranchId, destBranchId, RateMaster.CATEGORY_TYPE_GENERAL_ID, vehicleTypeId, 0, 0, chargeTypeId, 0, chargeSectionId, packingGrpTypeId, 0, consigneeId, operationalBranchId);
		checkAndInsertRates(rateMaster, rateId_mast, userWeightInput ,1);
		
		rateMaster = filterRates(srcBranchId, destBranchId, RateMaster.CATEGORY_TYPE_GENERAL_ID, vehicleTypeId, packingTypeId, 0, chargeTypeId, 0, chargeSectionId, packingGrpTypeId, consignmentGoodsId, 0, operationalBranchId);
		checkAndInsertRates(rateMaster, rateId_mast, userWeightInput ,1);
		
		rateMaster = filterRates(srcBranchId, destBranchId, RateMaster.CATEGORY_TYPE_GENERAL_ID, vehicleTypeId, packingTypeId, 0, chargeTypeId, 0, chargeSectionId, packingGrpTypeId, 0, 0, operationalBranchId);
		checkAndInsertRates(rateMaster, rateId_mast, userWeightInput ,1);
		
		rateMaster = filterRates(srcBranchId, destBranchId, RateMaster.CATEGORY_TYPE_GENERAL_ID, vehicleTypeId, 0, 0, chargeTypeId, 0, chargeSectionId, packingGrpTypeId, 0, 0, operationalBranchId);
		checkAndInsertRates(rateMaster, rateId_mast, userWeightInput ,1);

		//Loading type charges
		rateMaster = filterRates(srcBranchId, 0, RateMaster.CATEGORY_TYPE_GENERAL_ID, 0, 0, 0, chargeTypeId, 0, chargeSectionId, packingGrpTypeId, 0, 0, operationalBranchId);
		checkAndInsertRates(rateMaster, rateId_mast, userWeightInput, 1);
		...
		//Filter Slab Wise Rate
		rateMaster = filterSlabRates(srcBranchId, destBranchId, RateMaster.CATEGORY_TYPE_GENERAL_ID, vehicleTypeId, 0, corporateAccountId, chargeTypeId, rateTypeId, 0, 0, 0, consigneeId, operationalBranchId);
		checkAndInsertRates(rateMaster, rateId_mast, userWeightInput, 1);
		
		//Filter Slab Wise Rate
		rateMaster = filterSlabRates(srcBranchId, destBranchId, RateMaster.CATEGORY_TYPE_GENERAL_ID, vehicleTypeId, 0, corporateAccountId, chargeTypeId, rateTypeId, 0, 0, 0, 0, operationalBranchId);
		checkAndInsertRates(rateMaster, rateId_mast, userWeightInput, 1);
	}
...
function getPackTyprRates(srcBranchId, destBranchId, vehicleTypeId, corporateAccountId, categoryTypeId, packingTypeId, packingGrpTypeId, userArticleInput, rateTypeId, consignmentGoodsId, consigneeId, operationalBranchId) {
	...
	rateMaster = filterRates(srcBranchId, 0, categoryTypeId, vehicleTypeId, packingTypeId, corporateAccountId, chargeTypeId, rateTypeId, chargeSectionId, packingGrpTypeId, consignmentGoodsId, consigneeId, operationalBranchId);
	checkSpecificArticleRate(rateMaster);
	checkAndInsertRates(rateMaster, rateId_mast, userArticleInput, 1);
	
	//Specific Packing Type Rates (Only Loading Type level charges)
	rateMaster = filterRates(srcBranchId, 0, categoryTypeId, vehicleTypeId, packingTypeId, corporateAccountId, chargeTypeId, rateTypeId, chargeSectionId, packingGrpTypeId, 0, consigneeId, operationalBranchId);
	checkSpecificArticleRate(rateMaster);
	checkAndInsertRates(rateMaster, rateId_mast, userArticleInput, 1);
	...
	//Specific Packing Type Rates (Only Loading Type level charges)
	rateMaster = filterRates(srcBranchId, 0, categoryTypeId, vehicleTypeId, packingTypeId, corporateAccountId, chargeTypeId, rateTypeId, chargeSectionId, packingGrpTypeId, 0, 0, operationalBranchId);
	checkSpecificArticleRate(rateMaster);
	checkAndInsertRates(rateMaster, rateId_mast, userArticleInput, 1);
	...
	rateMaster = filterRates(srcBranchId, 0, categoryTypeId, vehicleTypeId, packingTypeId, corporateAccountId, chargeTypeId, rateTypeId, chargeSectionId, packingGrpTypeId, consignmentGoodsId, 0, operationalBranchId);
	checkSpecificArticleRate(rateMaster);
	checkAndInsertRates(rateMaster, rateId_mast, userArticleInput, 1);
	...
	//Filter Slab Wise Rate
	rateMaster = filterSlabRates(srcBranchId, destBranchId, categoryTypeId, vehicleTypeId, packingTypeId, corporateAccountId, chargeTypeId, rateTypeId, 0, 0, 0, consigneeId, operationalBranchId);
	checkSpecificArticleRate(rateMaster);
	checkAndInsertRates(rateMaster, rateId_mast, userArticleInput, 1);
	
	//Filter Slab Wise Rate
	rateMaster = filterSlabRates(srcBranchId, destBranchId, categoryTypeId, vehicleTypeId, packingTypeId, corporateAccountId, chargeTypeId, rateTypeId, 0, 0, 0, 0, operationalBranchId);
	checkSpecificArticleRate(rateMaster);
	checkAndInsertRates(rateMaster, rateId_mast, userArticleInput, 1);
	...
	if(categoryTypeId == RateMaster.CATEGORY_TYPE_PARTY_ID) {
		...
		//Specific Packing Type Rates (Only Loading Type level charges)
		rateMaster = filterRates(srcBranchId, 0, RateMaster.CATEGORY_TYPE_GENERAL_ID, vehicleTypeId, packingTypeId, 0, chargeTypeId, 0, chargeSectionId, packingGrpTypeId, consignmentGoodsId, 0, operationalBranchId);
		checkSpecificArticleRate(rateMaster);
		checkAndInsertRates(rateMaster, rateId_mast, userArticleInput, 1);
		...
		//Generic Packing Type Rates (Only Loading Type level charges)
		rateMaster = filterRates(srcBranchId, 0, RateMaster.CATEGORY_TYPE_GENERAL_ID, vehicleTypeId, packingTypeId, 0, chargeTypeId, 0, chargeSectionId, packingGrpTypeId, 0, 0, operationalBranchId);
		checkSpecificArticleRate(rateMaster);
		checkAndInsertRates(rateMaster, rateId_mast, userArticleInput, 1);
		
		//Filter Slab Wise Rate
		rateMaster = filterSlabRates(srcBranchId, destBranchId, RateMaster.CATEGORY_TYPE_GENERAL_ID, vehicleTypeId, packingTypeId, corporateAccountId, chargeTypeId, rateTypeId, 0, 0, 0, consigneeId, operationalBranchId);
		checkSpecificArticleRate(rateMaster);
		checkAndInsertRates(rateMaster, rateId_mast, userArticleInput, 1);
		
		//Filter Slab Wise Rate
		rateMaster = filterSlabRates(srcBranchId, destBranchId, RateMaster.CATEGORY_TYPE_GENERAL_ID, vehicleTypeId, packingTypeId, corporateAccountId, chargeTypeId, rateTypeId, 0, 0, 0, 0, operationalBranchId);
		checkSpecificArticleRate(rateMaster);
		checkAndInsertRates(rateMaster, rateId_mast, userArticleInput, 1);
	}
...
function calculateWeightTypeRates(chargedWeight, typeofPacking, consignmentGoodsId) {
	...
	srcBranchId				= executive.branchId;
	operationalBranchId		= $('#sourceBranchId').val();
	...
	if(configuration.allowCombineBranchRateInManualLR == 'true') {
		if(isManualWayBill) {
			wayBillRates = comibineRates(srcBranchId, destBranchId, vehicleTypeId, corporateAccountId, typeofPacking, operationalBranchId, $('#chargeType').val());
			if(wayBillRates == null || jQuery.isEmptyObject(wayBillRates)) {
				wayBillRates = comibineRates(srcBranchId, destBranchId, vehicleTypeId, 0, typeofPacking, operationalBranchId, $('#chargeType').val());
			}
		}
	}

	if($('#wayBillType').val() != WayBillType.WAYBILL_TYPE_FOC) {
		var response =  getWghtTypeRates(srcBranchId, destBranchId, vehicleTypeId, corporateAccountId, categoryTypeId, typeofPacking, packingGrpTypeId, chargedWeight, rateTypeId, consignmentGoodsId, consigneeId, operationalBranchId);
...
function getFreightChargeForRates(response) {
	...
	if(configuration.allowCombineBranchRateInManualLR == 'true') {
		if(response[BookingChargeConstant.FREIGHT].rate != undefined && response[BookingChargeConstant.FREIGHT].rate != 'undefined') {
			amnt	= Number(response[BookingChargeConstant.FREIGHT].rate);
		} else {
			amnt	= Number(response[BookingChargeConstant.FREIGHT]);
		}
	}
...
function calculatePackingTypeRates(quantity, typeofPacking,consignmentGoodsId){
	...
	if(typeofPacking > 0 && $('#wayBillType').val() != WayBillType.WAYBILL_TYPE_FOC) {
		...
		var response = getPackTyprRates(srcBranchId, destBranchId, vehicleTypeId, corporateAccountId, categoryTypeId, typeofPacking, packingGrpTypeId, quantity, rateTypeId, consignmentGoodsId, consigneeId, operationalBranchId);
...
function comibineRates(srcBranchId, destBranchId, vehicleTypeId, corporateAccountId, typeofPacking, operationalBranchId, chargeTypeId) {
	var rateId_mast			= new Object();
	var combineRates		= new Array();
	if(!jQuery.isEmptyObject(combinationWayBillRates)) {
		for(var i = 0; i < combinationWayBillRates.length; i++) {
			
			if((rateId_mast[combinationWayBillRates[i].chargeTypeMasterId] == null 
					|| (typeof rateId_mast[combinationWayBillRates[i].chargeTypeMasterId] == 'undefined'))
					&& combinationWayBillRates[i].chargeTypeId == chargeTypeId
					&& combinationWayBillRates[i].vehicleTypeId == vehicleTypeId
					&& combinationWayBillRates[i].chargeTypeId == chargeTypeId
					&& combinationWayBillRates[i].corporateAccountId == corporateAccountId
					&& combinationWayBillRates[i].packingTypeId == typeofPacking
					&& combinationWayBillRates[i].branchId == operationalBranchId) {
				rateId_mast[combinationWayBillRates[i].chargeTypeMasterId] = combinationWayBillRates[i];
				combineRates.push(combinationWayBillRates[i]);
			} 
		}
	}
	...
	if(!jQuery.isEmptyObject(combinationWayBillRates)) {
		for(var i = 0; i < combinationWayBillRates.length; i++) {
			if((rateId_mast[combinationWayBillRates[i].chargeTypeMasterId] == null 
					...
					&& combinationWayBillRates[i].corporateAccountId == corporateAccountId
					&& combinationWayBillRates[i].packingTypeId == typeofPacking
					&& combinationWayBillRates[i].branchId == srcBranchId) {
				combineRates.push(combinationWayBillRates[i]);
			} 
		}
	}
...
function combineLrLevelCharges(corporateAccountId) {

	var lrRateId_mast				= new Object();
	var combineLrRates				= new Array();
	if(!jQuery.isEmptyObject(chargesConfigRates1)) {
		for(var i = 0; i < chargesConfigRates1.length; i++) {
			if((lrRateId_mast[chargesConfigRates1[i].chargeTypeMasterId] == null 
					|| (typeof lrRateId_mast[chargesConfigRates1[i].chargeTypeMasterId] == 'undefined'))
					&& chargesConfigRates1[i].corporateAccountId == corporateAccountId
					&& chargesConfigRates1[i].branchId == $('#sourceBranchId').val()) {
				lrRateId_mast[chargesConfigRates1[i].chargeTypeMasterId] = chargesConfigRates1[i];
				combineLrRates.push(chargesConfigRates1[i]);
			} 
		}
	}
	...
	if(!jQuery.isEmptyObject(chargesConfigRates1)) {
		for(var i = 0; i < chargesConfigRates1.length; i++) {
			if((lrRateId_mast[chargesConfigRates1[i].chargeTypeMasterId] == null 
					|| (typeof lrRateId_mast[chargesConfigRates1[i].chargeTypeMasterId] == 'undefined'))
					&& chargesConfigRates1[i].corporateAccountId == corporateAccountId
					&& chargesConfigRates1[i].branchId == executive.branchId) {
				combineLrRates.push(chargesConfigRates1[i]);
			} 
		}
	}
	
	return combineLrRates;
}
