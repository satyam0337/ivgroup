...
function checkRateForPackingType(qtyAmount, quantity ) {
	...
	if(searchRate == "") {
		...
		for(var j=0;j<qtyRateFromEle.length;j++){
			...
			if(qtyRateFromEle[j].split("=")[0] == FREIGHT) {
		...
		if(!isFreightRateExist) {
			...
			if(tempString != "") {
				tempString += ","+FREIGHT+'='+parseFloat(finalQtyRate);
			} else {
				tempString = FREIGHT+'='+parseFloat(finalQtyRate);
			};
		}
...
function calculateChargedWeight(obj) {
	...
	if(configuration.IsDisplayAlertToSelectLCV == 'true') {
		if(Number(actualWeight) >= configuration.MaxWeightToShowAlertForLCV && getBookingType() == BOOKING_TYPE_SUNDRY_ID) {
			...
			for (var i = 0; i < chargesConfigRates.length; i++) {
				...
				if(Number(rates.accountGroupId) 			== Number(accountGroupId)
					...
					if(corporateAccountId <= 0 && configuration.doNotApplyBranchRateIfArtType == 'true') {
						...
						/*if((configuration.doNotApplyBranchRateIfArtType == 'false' ||  Number($('#chargeType').val()) != CHARGETYPE_ID_QUANTITY || chargeIdToAvoid.includes(Number(rates.chargeTypeMasterId)))){
							arr.push(chargesConfigRates[i]);	
						}*/
					} else {
						arr.push(chargesConfigRates[i]);
...
function calculateWeightTypeRates(chargedWeight, typeofPacking, consignmentGoodsId) {
	if(configuration.applyRateAuto == 'false' || $('#wayBillType').val() == WAYBILL_TYPE_FOC || configuration.doNotAllowToBookTopayLRWithAmount == 'true' && $('#wayBillType').val() == WAYBILL_TYPE_TO_PAY)
		return;
	...
	if(typeofPacking > 0) {
		...
		if(response && !jQuery.isEmptyObject(response)) {
			if(applyRateForSpecificArticle && noOfArticlesAdded == 0 && chargeTypeId != CHARGETYPE_ID_QUANTITY) { // 3 is Article Type
				...
				if(chargeTypeId == CHARGETYPE_ID_QUANTITY || isTokenWayBill) {
					qtyAmt 				= getFreightChargeForRates(response);
				}
			}
...
function calculateLBHOnChargeType(obj) {
	...
	if(chargeType.value == CHARGETYPE_ID_CUBIC_RATE || (configuration.VolumetricRate) == 'true') {
		...
		if(lbhTotal.value > 0) {
			...
			} else if(configuration.defaultLBHOperator == LBH_OPERATOR_DIVIDE_ID) {
				$('#chargedWeight').val(Math.round((lbhTotal.value) / (configuration.defaultLBHValue)));
				if (configuration.TemporaryWght == 'true') {
					$('#tempWeight').val(Math.round((lbhTotal.value) / (configuration.defaultLBHValue)));
				}
...
function calcDiscountOnPercentage() {
	if(isBookingDiscountPercentageAllow) {
		...
		if(Number($("#discountPercentage").val()) <= Number(configuration.maximumDiscountValue)) {
			discountAmount = (parseFloat($("#discountPercentage").val()) * parseFloat($("#charge"+FREIGHT).val())) / 100;
...
function checkAndUpdateDiscountOnPercentage() {
	...
	if(isBookingDiscountPercentageAllow) {
		...
		discountPercentage = (parseFloat($("#discount").val()) / parseFloat($("#charge"+FREIGHT).val())) * 100;
...
function checkChargesRates(obj) {
	...
	if(configuration.editDeliveryAtBasedOnDlyChrges == 'true' && chargeMasterId == DOOR_DELIVERY_BOOKING) {
		if($('#charge'+chargeMasterId).val() > 0 && $('#deliveryTo').val() != DELIVERY_TO_DOOR_ID){
...
function calculateLRLevelCharges(chargesRates, chargeMasterId, masterIdsArr) {
	if(configuration.AllowEnableDoorDelivery == 'true') {
		if($("#deliveryTo").val() ==  DELIVERY_TO_DOOR_ID && Number($('#wayBillType').val()) != Number(WAYBILL_TYPE_FOC)) {
...
function applyRates() {
	...
	if($('#charge'+LR_CHARGE).val() <= 0)
		$('#charge'+LR_CHARGE).val(configuration.LRCharge);
	
	if($('#charge'+STATISTICAL).val() <= 0)
		$('#charge'+STATISTICAL).val(configuration.defaultStatisticalCharge);
...
function calculateCustomChargesAmount(chargeMasterId, chargeValue, chargeUnit) {
	...
	if(chargeUnit > 0) {
		if(chargeUnit == CHARGE_TYPE_UNIT_WEIGHT) {
			if ($("#chargedWeight").val() > 0) {
				chargeAmount = (chargeValue * $("#chargedWeight").val());
			}
		} else if(chargeUnit == CHARGE_TYPE_UNIT_QUANTITY) {
			var totalQty	= 0;
			
			if(configuration.isPackingGroupTypeWiseChargeAllow == 'true' && chargeMasterId == AOC) {
...
function getFlavourWiseRates(customerId, partyType) {
	...
	if($('#wayBillType').val() == WAYBILL_TYPE_CREDIT && customerId > 0) {
...
function validateInsuranceCharge() {
	if(!isManual && configuration.isIncreaseCRInsuranceChargeAllow == 'true') {
		var bookingDestBranchId  						= $('#destinationBranchId').val();
		var sourceBranchListForAvoidIncreaseCRInsurance	= configuration.sourceBranchListForAvoidIncreaseCRInsurance;
		var destinationBranchListForIncreaseCRInsurance	= configuration.destinationBranchListForIncreaseCRInsurance;
		var destBranchListForAvoidIncreaseCRInsurance	= configuration.destBranchListForAvoidIncreaseCRInsurance;

		var destBranchToIncreaseArr = destinationBranchListForIncreaseCRInsurance.split(',').map(function(item) {
			return parseInt(item.trim(), 10);
		});

		var sourceBranchToAvoidArr = sourceBranchListForAvoidIncreaseCRInsurance.split(',').map(function(item) {
			return parseInt(item.trim(), 10);
		});
		var destBranchToAvoidArr = destBranchListForAvoidIncreaseCRInsurance.split(',').map(function(item) {
			return parseInt(item.trim(), 10);
		});

		if(bookingDestBranchId != undefined) {
			if (checkAddCarreirAllow(sourceBranchToAvoidArr,destBranchToAvoidArr,Number(bookingDestBranchId),destBranchToIncreaseArr)){
				var ele				= document.getElementById('charge' + CR_INSUR_BOOKING);
				
				if(ele) {
					var carreirRiskCurrentCharge	= parseFloat(ele.value);
					var carreirRiskDfaultCharge	= Number($('#insuranceChargeValue').val());
					
					if(carreirRiskDfaultCharge > 0) {
						if(isNaN(carreirRiskCurrentCharge) || carreirRiskCurrentCharge == '' || carreirRiskCurrentCharge < carreirRiskDfaultCharge) {
							showMessage('error', 'You can not enter CR/Insurance Charge Less Than ' + carreirRiskDfaultCharge + '/-');
							changeError1(ele.id);
...
function calculateChargeMinAmount(chargeMasterId,chargeMinAmount,chargeApplicableOnId,filter){
	...
	if((configuration.compareFOVWithDecVal == 'true' || configuration.compareFOVWithDecVal == true) && chargeMasterId == FOV && decVal < 200000) {
		return 0;
	}
...
function checkAddCarreirAllow(sourceBranchToAvoidArr,destBranchToAvoidArr,bookingDestBranchId,destBranchToIncreaseArr) {

	if(sourceBranchToAvoidArr.includes(branchId)){
		if(destBranchToIncreaseArr.includes(bookingDestBranchId)){
			if(destBranchToAvoidArr.includes(bookingDestBranchId)){
				return false;
			}else{
				return true;
			}
...
function enableInsuranceChargeForBranch() {
	var isAddCarreirAllow;

	if(configuration.enableCrInsuranceCharge == 'true') {
		if(!isManual) {
			var bookingDestBranchId  = $('#destinationBranchId').val();
			var sourceBranchListForAvoidIncreaseCRInsurance	= configuration.sourceBranchListForAvoidIncreaseCRInsurance;
			var destinationBranchListForIncreaseCRInsurance	= configuration.destinationBranchListForIncreaseCRInsurance;
			var destBranchListForAvoidIncreaseCRInsurance	= configuration.destBranchListForAvoidIncreaseCRInsurance;
			var	amountToBeAddedInCRInsuranceCharge			= configuration.amountToBeAddedInCRInsuranceCharge;
			...
			if(bookingDestBranchId != undefined) {
				...
				isAddCarreirAllow	=	checkAddCarreirAllow(sourceBranchToAvoidArr, destBranchToAvoidArr, Number(bookingDestBranchId), destBranchToIncreaseArr);

				if(isAddCarreirAllow) {
					var totalQuantity		= getTotalAddedArticleTableQuantity();

					if(totalQuantity > 0) {
						if(checkAddCrInsuranceQuantityWise()) {
							$('#charge' + CR_INSUR_BOOKING).val(Number(totalQuantity * amountToBeAddedInCRInsuranceCharge));
							$('#insuranceChargeValue').val(Number(totalQuantity * amountToBeAddedInCRInsuranceCharge));
							calcTotal();
						}else{
							$('#charge' + CR_INSUR_BOOKING).val(Number(amountToBeAddedInCRInsuranceCharge));
							$('#insuranceChargeValue').val(Number(amountToBeAddedInCRInsuranceCharge));
							calcTotal();
						}
						$('#charge' + CR_INSUR_BOOKING).removeAttr("readonly");
					}
				} else {
					if(configuration.isIncreaseCRInsuranceChargeAllow == 'true'){
						$('#charge' + CR_INSUR_BOOKING).val(0);
						$('#charge' + CR_INSUR_BOOKING).prop("readonly", "true");
					}
				}
...
function checkAddCrInsuranceQuantityWise() {
	sourceBranchForQuantityWiseCRInsuranceArr	=  configuration.sourceBranchForQuantityWiseCRInsurance.split(',').map(function(item) {
		return parseInt(item.trim(), 10);
	});
	if(sourceBranchForQuantityWiseCRInsuranceArr.includes(branchId)){ //branchId login branchid
		return true;
	}else{
		return false;
	}
