			...
			$(".ok").on('click', function() {
				...
				if(myNod.areAll('valid')) {
					...
					if(bill.billBillId > 0) {
						...
						jsonObjectNew["previousHSNCode"] 			= bill.previousHSNCode;
						jsonObjectNew["hsnCode"] 					= $('#hsnCode').val();
						jsonObjectNew["previousExecutiveId"] 		= bill.billExecutiveId;
					}
	
					btModalConfirm = new Backbone.BootstrapModal({
						content		: 	"Are you sure you want to Update HSN Code?",
						modalWidth 	: 	30,
					...
					btModalConfirm.on('ok', function() {
						getJSON(jsonObjectNew, WEB_SERVICE_URL+'/creditorInvoiceWS/updateBillHSNCode.do', _this.onUpdate, EXECUTE_WITH_ERROR); //submit JSON
						showLayer();
					});
				} else {
					return false;
