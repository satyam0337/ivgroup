(function($){
	...
	$.linguaLoad=function(language){
		return doLoad(language)
	}
	function doLoad(language){
		if(typeof lingua__path!='string'||typeof lingua__name!='string'){
			...
			var query;
			var file="";
			if(lang.length>=5){query=$.ajax($.extend(opts,{url:lingua__path+lingua__name+'-'+lang.substring(0,5)+'.txt'}));
			if(query.status==200)file=query.responseText;}
			if(file.length==0&&lang.length>=2){
				query=$.ajax($.extend(opts,{url:lingua__path+lingua__name+'-'+lang.substring(0,2)+'.txt'}));if(query.status==200)file=query.responseText;}
			if(file.length==0){
				query=$.ajax($.extend(opts,{url:lingua__path+lingua__name+'.txt'}));if(query.status==200)file=query.responseText;
			}
			...
			for(var i=0;i<rawText.length;i++){
				...
				if(sep>=0){
					var datkey=$.trim(row.slice(0,sep));
					if(datkey.substring(0,2)!="//"&&datkey.substring(0,3)!="﻿"){if(datkey.charAt(0)=="#"){
						lingua__eids[datkey]=$.trim(row.slice(sep+1));	
					}else{
	...
	$.linguaUpdateElements=function(){
		return doUpdateElements()
	}
	function doUpdateElements(){
		/*$.each(lingua__eids,function(key,value){
			$(key).html(value.replace(/ /g, "&nbsp;"));});*/
		$.each(lingua__data,function(key,value){
			$("*[data-selector='"+key+"']").html(value.replace(/ /g, "&nbsp;"));
		});
		return lingua__data;
	}
	...
	function normalizeLang(lang){
		...
		if(lang.length>3)lang=lang.substring(0,3)+lang.substring(3).toUpperCase();return lang;}
})(jQuery);