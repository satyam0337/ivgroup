	...
	$.getJSON("Ajax.do?pageId=9&eventId=15",
			{json:jsonStr,filter:4}, function(data) {
				if (!data || jQuery.isEmptyObject(data) || data.errorDescription) {
					...
					consignorChargesConfigRates	= data.consignorChargesConfigRates;
					cnorWayBillRates			= data.cnorWayBillRates;
...
function applyRates() {
	...
	if(chargesConfigRates != null && chargesConfigRates.length > 0){
		for(var i = 0;i<chargesConfigRates.length;i++){
			if(chargesConfigRates[i].chargeTypeMasterId == BookingChargeConstant.LR_CHARGE){
				if(chargesConfigRates[i].corporateAccountId > 0){
					if(chargesConfigRates[i].applicable == 'false' || chargesConfigRates[i].applicable == false ){
						$('#charge'+BookingChargeConstant.LR_CHARGE).val(0);
						$('#charge' + BookingChargeConstant.LR_CHARGE).removeAttr('disabled');
					} else{
						if($('#charge'+BookingChargeConstant.LR_CHARGE).val() <= 0) {
							$('#charge'+BookingChargeConstant.LR_CHARGE).val(configuration.LRCharge);
						}
