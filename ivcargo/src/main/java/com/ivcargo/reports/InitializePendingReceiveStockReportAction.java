package com.ivcargo.reports;

import java.util.HashMap;
import java.util.LinkedHashMap;

import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import com.framework.Action;
import com.iv.utils.dataObject.ValueObject;
import com.ivcargo.actionUtility.ActionInstanceUtil;
import com.ivcargo.actionUtility.ActionStaticUtil;
import com.ivcargo.actionUtility.ActionStepsUtil;
import com.ivcargo.utils.CacheManip;
import com.platform.dto.Executive;
import com.platform.dto.SubRegion;


@Deprecated
public class InitializePendingReceiveStockReportAction implements Action{

	@Override
	public void execute(HttpServletRequest request, HttpServletResponse response) {
		HashMap<String,Object>	error 			= null;
		ActionInstanceUtil 		actionUtil2 	= null;
		Executive   			executive 		= null;

		try {
			error = ActionStaticUtil.getSystemErrorColl(request);

			if(ActionStaticUtil.isSystemError(request,error))
				return;

			final CacheManip cacheManip	= new CacheManip(request);

			request.setAttribute(ActionStaticUtil.IS_ALL_BRANCHES_NEED_TO_SHOW,true);
			request.setAttribute(ActionStaticUtil.IS_ALL_CITY_NEED_TO_SHOW, false);

			request.setAttribute("cityBranchLabel", "From Branch");

			executive = cacheManip.getExecutive(request);

			ActionStepsUtil.setExecutiveCoreReportPermissionsInRequest(request, executive);

			actionUtil2 = new ActionInstanceUtil();
			actionUtil2.setExecutiveTypeBooleanInRequest(request, executive, true);

			if(executive.getExecutiveType() == Executive.EXECUTIVE_TYPE_GROUPADMIN) {

				final SubRegion[] subRegion 		= cacheManip.getSubRegionsByGroupId(request, executive.getAccountGroupId());
				final ValueObject subRegionObject 	= cacheManip.getAllSubRegions(request);
				final HashMap<Long, String> subRegionList = new LinkedHashMap<Long, String>();

				for (final SubRegion element : subRegion)
					subRegionList.put(element.getSubRegionId(), ((SubRegion) subRegionObject.get(element.getSubRegionId())).getName());

				request.setAttribute("subRegionList", subRegionList);
			}

			request.setAttribute("wayBillType", cacheManip.getAllWayBillType(request));
			request.setAttribute("nextPageToken", "success");
		} catch (final Exception e) {
			ActionStepsUtil.catchActionException(request, e, error);
		} 	finally{
			error 			= null;
			actionUtil2 	= null;
			executive 		= null;
		}
	}
}
