...
var sessionTimer = self.setInterval(function(){
	if(configuration != null && (configuration.applyDiscountThroughMaster  == 'true' || configuration.applyDiscountThroughMaster  == true)) { 
		refreshSession();
	}				
},10*1000);
...
function refreshSession(){
	...
	if(noRecords == false && discountTimeList != undefined){
		for(var j = 0; j < discountTimeList.length;j++){
			var startTimeMilisecond = discountTimeList[j].startTimeMilisecond;
			var endTimeMilisecond   = discountTimeList[j].endTimeMilisecond;
...
function editChargedWeight(obj) {
	...
	if(actualWeight <= minWeight) {
		if(chargedWeight < minWeight && !$('#volumetric').is(':checked')) {
...
function calculateChargedWeight(obj) {
	...
	if(isWeightAllow) {		
		...
		if(bookingType == BookingTypeConstant.BOOKING_TYPE_FTL_ID && configuration.isAllowToExceedVehicleCapacityBeyondLimit == 'true') {
...
function calculateWeigthRate() {
	...
	if(chargedWeight != 0 && weigthFreightRate != 0) {
		...
		$('#weightAmount').val(weigthFreightRate * chargedWeight);
}
...
function getFilteredChargesToCalculateFSCharge() {
	...
	if(chargesConfigRates != null) {
		for(var i = 0; i < chargesConfigRates.length;i++) {
			...
			if(applicableOnChargeTypeMasterIds == null && chargesConfigRates[i].branchId == executive.branchId) {
				applicableOnChargeTypeMasterIds	= chargesConfigRates[i].applicableOnChargeTypeMasterIds;
				chargeMinAmountforFSCharge		= chargesConfigRates[i].chargeMinAmount;
			}
		}
...
function getRates(destBranchId, corporateAccountId, partyTypeId) {
	...
	var chargeTypeIdFromRateMaster	= new Array();
			...
			{json:jsonStr,filter:4}, function(data) {
				...
				if (!data || jQuery.isEmptyObject(data) || data.errorDescription) {
					...
					//if(freightRatePartyId > 0) {
						...
						if(data.businessTypeList != undefined && (data.businessTypeList).length > 0)
							createBusinessTypeOption(data.businessTypeList);
						else {
					...
					if(configuration.allowConsigorRatesOnTopayLR == 'true' && !showRatesNotFound) {
						for(var i = 0; i < wayBillRates.length; i++) {
							if(Number(corporateAccountId) > 0 && Number(wayBillRates[i].corporateAccountId) == Number(corporateAccountId))
...
function filterRates(srcBranchId, destBranchId, categoryTypeId, vehicleTypeId, packingTypeId, corporateAccountId, chargeTypeId, rateTypeId, chargeSectionId, packingGrpTypeId, consignmentGoodsId, consigneeId) {
	...
	if(configuration.businessTypeSelection == 'true' && $('#businessType').exists())
	...
	if(!jQuery.isEmptyObject(wayBillRates)) {
		for (var i = 0; i < wayBillRates.length; i++) {
			...
			if(chargeSectionId == RateMaster.CHARGE_SECTION_FIXED_RATE_ID) { //fixed rate
				...
				&& (Number($('#cftUnit').val()) == Number(datarates.cftUnitId) || Number(datarates.cftUnitId) == 0)
				...
				&& (Number(datarates.businessTypeId) 		== Number(businessTypeId) || Number(datarates.businessTypeId) == 0)
			) {
				arr.push(wayBillRates[i]);
			}
		}
	...
	case 1:
		if(rateMaster != null && rateMaster.length > 0) {
			for (var i = 0; i < rateMaster.length; i++) {
				if((rateId_mast[rateMaster[i].chargeTypeMasterId] == null || (typeof rateId_mast[rateMaster[i].chargeTypeMasterId] == 'undefined') || rateId_mast[rateMaster[i].chargeTypeMasterId] == 0) && rateMaster[i].chargeSectionId != 5 && rateMaster[i].chargeSectionId != 6) {
				...
				if(minimumValueConfigHM[rateMaster[i].chargeSectionId] == undefined)
					minimumValueConfigHM[rateMaster[i].chargeSectionId]    = rateMaster[i].rate;
			}
		}
		break;
...
function getWeightTypeRates() {
    ...
    || Number($("#chargeType").val()) != CHARGETYPE_ID_WEIGHT)
        return;
...
function calculateWeightTypeRates(chargedWeight, typeofPacking, consignmentGoodsId) {
	...
	if($('#wayBillType').val() != WayBillType.WAYBILL_TYPE_FOC) {
		...
		if(response && !jQuery.isEmptyObject(response)) {
			...
			if (minimumValueConfigHM.hasOwnProperty(CHARGE_SECTION_PARTY_NAME_MINIMUM_RATE_ID)) {
				let fixedChargeRate = minimumValueConfigHM[CHARGE_SECTION_PARTY_NAME_MINIMUM_RATE_ID];
				
				if (fixedChargeRate > weightFromDb) {
					weightFromDb = fixedChargeRate;
					//response[FREIGHT] = fixedChargeRate;
				}
			}
			
			if (minimumValueConfigHM.hasOwnProperty(CHARGE_SECTION_PARTY_MINIMUM_WEIGHT_ID)) {
				let minChargedWeight = minimumValueConfigHM[CHARGE_SECTION_PARTY_MINIMUM_WEIGHT_ID];
				
				if (minChargedWeight > chargedWeight) {
					chargedWeight	= minChargedWeight;
...
function calculatePackingTypeRates(quantity, typeofPacking, consignmentGoodsId) {
	...
	if(typeofPacking > 0 && $('#wayBillType').val() != WayBillType.WAYBILL_TYPE_FOC) {
		...
		if(response && !jQuery.isEmptyObject(response)) {
			if(applyRateForSpecificArticle && noOfArticlesAdded == 0 && chargeTypeId != ChargeTypeConstant.CHARGETYPE_ID_QUANTITY) {
			 // 3 is Article Type
			...
			if (minimumValueConfigHM.hasOwnProperty(CHARGE_SECTION_PARTY_NAME_MINIMUM_RATE_ID)) {
				...
				if (fixedChargeRate > qtyAmt) {
					qtyAmt = fixedChargeRate;
					//response[FREIGHT] = fixedChargeRate;
				}
			}
...
function partyMinimumRates() {
	...
	if(partyWiseMinimumValueDataHM == undefined)
		return 0;

	let corporateAccountId		= getPartyIdForMinimum();
	let minimumValueRateList	= partyWiseMinimumValueDataHM[corporateAccountId];
	let businessTypeId			= $('#businessType').val();
	
	if(minimumValueRateList != undefined) {
		for(const element of minimumValueRateList) {
			if(element.chargeSectionId == CHARGE_SECTION_PARTY_NAME_MINIMUM_RATE_ID 
	...
	if($('#wayBillType').val() != WAYBILL_TYPE_FOC) {
		let minAmt	= Number(jsonPartyMinAMt[corporateAccountId]);
		
		if(minAmt) { 
			$('#partyMinimumRate').val(Number(minAmt));
			return Number(minAmt);
		}
	}
...
function partyMinimumWeight() {
	...
	if(minimumValueRateList != undefined) {
		for(let i = 0; i < minimumValueRateList.length; i++) {
			if(minimumValueRateList[i].chargeSectionId == CHARGE_SECTION_PARTY_MINIMUM_WEIGHT_ID 
				&& (businessTypeId == minimumValueRateList[i].businessTypeId))
				return minimumValueRateList[i].rate;
		}
	}
...
function setPartyMinimumRates() {
	var minimumParty	= partyMinimumRates();
...
function applyRates() {
	...
	if($('#volumetric').is(':checked') &&  $('#fixAmount').val() > 0){
		if(consigAddedtableRowsId.length > 1)
			$("#charge" + FREIGHT).val(Number($('#fixAmount').val()) + Number($('#weightAmount').val()));
		else
			$("#charge" + FREIGHT).val($('#fixAmount').val());		
	}
...
function calculateCustomChargesAmount(chargeMasterId, chargeValue, chargeUnit) {
	...
	if(chargeMasterId == BookingChargeConstant.LOADING && configuration.isPackingGroupWiseRateApplicable == 'true') {
...
function calculateFSOnOtherCharge(chargeTypeMasterId) {
	if(configuration.calcFsRate == 'true' || configuration.calcFsRate == true) {
		var fsRateCalculateOnAMt	= 0;

		if(applicableOnChargeTypeMasterIdArr != null) {
			for(var i = 0; i < applicableOnChargeTypeMasterIdArr.length; i++) {
				fsRateCalculateOnAMt	+= Number($('#charge' + applicableOnChargeTypeMasterIdArr[i]).val());
			}
...
function filterCFTValueOnCFTUnit() {
	if(wayBillRates == null) {
	...
	var chargeTypeId		= Number($('#chargeType').val());
	var vehicleTypeValue	= $('#vehicleType').val();
	var consigneePartyId	= $('#consigneePartyMasterId').val();
	var cftUnit				= $('#cftUnit').val();
	var wayBillTypeId		= Number($('#wayBillType').val());
	...
	wayBillNewRates	= wayBillRates.filter(function(e) {
		return e.branchId == srcBranchId && e.chargeTypeId == chargeTypeId && e.cftUnitId == cftUnit
		&& e.vehicleTypeId == vehicleTypeId && e.businessTypeId == Number(businessTypeId)
		&& e.corporateAccountId	== freightRatePartyId && e.wayBillTypeId == wayBillTypeId
		&& (chargeTypeId == CHARGETYPE_ID_QUANTITY && e.packingTypeId == typeofPacking && e.consignmentGoodsId == consignmentGoodsId || chargeTypeId == CHARGETYPE_ID_WEIGHT)
		&& e.consigneePartyId == consigneePartyId
	});
	
	if(wayBillNewRates.length == 0) {
		wayBillNewRates	= wayBillRates.filter(function(e) {
			return e.branchId == srcBranchId && e.chargeTypeId == chargeTypeId && e.cftUnitId == cftUnit
			&& e.vehicleTypeId == vehicleTypeId && e.businessTypeId == Number(businessTypeId)
			&& e.corporateAccountId	== freightRatePartyId && e.wayBillTypeId == wayBillTypeId
			&& (chargeTypeId == CHARGETYPE_ID_QUANTITY && e.packingTypeId == typeofPacking && e.consignmentGoodsId == consignmentGoodsId || chargeTypeId == CHARGETYPE_ID_WEIGHT)
			&& e.consigneePartyId == 0
		});
	}
	...
	if(wayBillNewRates.length == 0) {
		wayBillNewRates	= wayBillRates.filter(function(e) {
			return e.branchId == srcBranchId && e.chargeTypeId == chargeTypeId && e.cftUnitId == cftUnit
			&& e.vehicleTypeId == 0 && e.businessTypeId == Number(businessTypeId)
			&& e.corporateAccountId	== freightRatePartyId && e.wayBillTypeId == wayBillTypeId
			&& (chargeTypeId == CHARGETYPE_ID_QUANTITY && e.packingTypeId == typeofPacking && e.consignmentGoodsId == 0 || chargeTypeId == CHARGETYPE_ID_WEIGHT)
			&& e.consigneePartyId == consigneePartyId
		});
	}
	...
	if(wayBillNewRates.length > 0)
		$('#cftRate').val(wayBillNewRates[0].cftRate);
}

function filterCFTFixAmountOnCFTUnit(typeofPacking, consignmentGoodsId, cftWeight) {
	var wayBillNewRates		= [];
	var srcBranchId			= getSourceBranchForRate();
	...
	if(configuration.rateApplicableOnSaidToContain == 'false')
		consignmentGoodsId	= 0;
	...
	for(let i = 0; i < wayBillNewRates.length; i++) {
		if(wayBillNewRates[i].isFixedCftAmt && cftWeight >= wayBillNewRates[i].minWeight && cftWeight <= wayBillNewRates[i].maxWeight) {
			return wayBillNewRates[i].rate;
		}
	}
