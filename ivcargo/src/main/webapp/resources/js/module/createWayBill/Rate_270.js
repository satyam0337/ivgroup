...
function setRateType() {
	...
	if(configuration.ChargeTypeFlavour == '2' && (currentWayBillTypeId == WAYBILL_TYPE_PAID || currentWayBillTypeId == WAYBILL_TYPE_CREDIT)
	|| configuration.ChargeTypeFlavour == '1')
		rateTypeId 	= CUSTOMER_TYPE_CONSIGNOR_ID;
	else
		rateTypeId 	= CUSTOMER_TYPE_CONSIGNEE_ID;

	$("#rateType").val(rateTypeId);
}
...
function getChargesRates() {
	...
	if(configuration.ChargeTypeFlavour == '1') {
		...
		if(configuration.subRegionWiseChargeTypeFlavour == 'true'
			&& checkSubRegionIdsForChargeTypeFlavour){
...
function getRates(destBranchId, corporateAccountId, partyTypeId) {
	...
	$.getJSON("RouteWiseRates.do?pageId=9&eventId=15",
			{json:jsonStr,filter:4}, function(data) {
				if (!data || jQuery.isEmptyObject(data) || data.errorDescription) {
					...
					if(Number($('#billingPartyId').val()) > 0 && configuration.resetTBBPartyDetailOnRateNotExist == 'true') {
						var resetTBBParty = false;
						for(var i = 0; i < wayBillRates.length;i++) {
							if(Number($('#billingPartyId').val()) > 0 && Number(wayBillRates[i].corporateAccountId) == Number($('#billingPartyId').val())) {
								if(Number(wayBillRates[i].chargeTypeMasterId) == Number(BookingChargeConstant.FREIGHT)) {
									resetTBBParty = true;
								}
							}
						}
...
//filter rate as par user selection get revert filterd data 
function filterPartyMinRates(srcBranchId, destBranchId, categoryTypeId, vehicleTypeId, packingTypeId, corporateAccountId, chargeTypeId, chargeSectionId) {
	var accountGroupId	= executive.accountGroupId;
	var arr 			= new Array();
		...
		if(rateMaster != null && rateMaster.length > 0) {
			...
			for (var i = 0; i < rateMaster.length; i++) {
				if(rateId_mast[rateMaster[i].chargeTypeMasterId] == null || (typeof rateId_mast[rateMaster[i].chargeTypeMasterId] == 'undefined')) {
					...
					if((rateMaster[i].chargedFixed) && rateMaster[i].rate > 0
							&&  jQuery.inArray(rateMaster[i].chargeTypeMasterId + "", fixRateChargeIds) != -1 )
						isApplyFixRate	=	true;
				} else {
					if(rateMaster[i].minWeight > 0 && rateMaster[i].maxWeight > 0) {
						if(configuration.isApplyFixRateonArticleRate == 'true' || configuration.isApplyFixRateonArticleRate == true) {
...
function getWghtTypeRates(srcBranchId, destBranchId, vehicleTypeId, corporateAccountId, categoryTypeId, packingTypeId, packingGrpTypeId, userWeightInput, rateTypeId, consignmentGoodsId, consigneeId) {
	...
	//Filter Fix Rate
	rateMaster = filterRates(srcBranchId, destBranchId, categoryTypeId, 0, 0, corporateAccountId, 0, rateTypeId, 0, 0, 0, consigneeId)
	checkAndInsertRates(rateMaster, rateId_mast, userWeightInput, 1);
	
	rateMaster = filterRates(srcBranchId, destBranchId, categoryTypeId, 0, 0, corporateAccountId, 0, rateTypeId, 0, 0, 0, 0)
	checkAndInsertRates(rateMaster, rateId_mast, userWeightInput, 1);
	...
	if(categoryTypeId == RateMaster.CATEGORY_TYPE_PARTY_ID ) {
		...
		//Filter Fix Rate
		rateMaster = filterRates(srcBranchId, destBranchId, RateMaster.CATEGORY_TYPE_GENERAL_ID, 0, 0, 0, 0, rateTypeId, 0, 0, 0, consigneeId)
		checkAndInsertRates(rateMaster, rateId_mast, userWeightInput, 1);
		
		//Filter Fix Rate
		rateMaster = filterRates(srcBranchId, destBranchId, RateMaster.CATEGORY_TYPE_GENERAL_ID, 0, 0, 0, 0, rateTypeId, 0, 0, 0, 0)
		checkAndInsertRates(rateMaster, rateId_mast, userWeightInput, 1);
	}
...
function partyMinRate(srcBranchId,destBranchId,corporateAccountId,categoryTypeId,rateTypeId) {
	...
	rateMaster = filterPartyMinRates(srcBranchId, destBranchId, categoryTypeId, 0, 0, corporateAccountId, 0, chargeSectionId);
	...
	if(jQuery.isEmptyObject(rateId_mast)) {
		rateMaster = filterPartyMinRates(srcBranchId, destBranchId, RateMaster.CATEGORY_TYPE_GENERAL_ID, 0, 0, 0, 0, chargeSectionId);
...
function getPartyIdForRate() {
	...
	} else if(configuration.ChargeTypeFlavour == '2' && (currentWayBillTypeId == WAYBILL_TYPE_PAID || currentWayBillTypeId == WAYBILL_TYPE_CREDIT)
	|| configuration.ChargeTypeFlavour == '1') {
		corporateAccountId	= $('#partyMasterId').val();
	} else {
		corporateAccountId	= $('#consigneePartyMasterId').val();
...
function calculateWeightTypeRates(chargedWeight, typeofPacking, consignmentGoodsId) {
	...
	let corporateAccountId	= getPartyIdForRate();
	let categoryTypeId		= CATEGORY_TYPE_PARTY_ID;
	let rateTypeId			= 0;
...
//	If Amount is not found from Rate Master Then applying Static Rates After checking the Following Conditions
	...
	if(configuration.subRegionWisedisableQantityAmountIfRateFound == 'true' && $('#wayBillType').val() != WayBillType.WAYBILL_TYPE_FOC) {
		var subRegionIdArray 		= configuration.subRegionIdsTodisableQantityAmountIfRateFound.split(',');
		
		if(isValueExistInArray(subRegionIdArray, executive.subRegionId) && qtyAmt > 0) {
			$('#qtyAmount').prop("disabled", "false");
			$('#add').focus();
		}
	}
}
...
function getTempStringForRates(response,quantity){
	...
	for (var key in response) {
		...
		if(configuration.ApplyArticleWiseRateOnWeightType == 'true' && getBookingType() == BookingTypeConstant.BOOKING_TYPE_SUNDRY_ID
			...
			if(isApplyFixRate && jQuery.inArray(chargeTypeId + "", fixRateChargeIds) != -1) {
				qtyAmt	= Number(response[key]);
			} else {
				qtyAmt	= Number(response[key]) * Number(quantity);
			}
		...
		if($('#chargeType').val() == ChargeTypeConstant.CHARGETYPE_ID_QUANTITY && configuration.applyRatesOnlyOnceOnLRForArticleType == 'true') {
			...
			for(var j = 0; j < sliptedByCommas.length; j++) {
				if(sliptedByCommas[j].split("=")[0] == chargeMasterId) {
					if(isApplyFixRate && jQuery.inArray(chargeMasterId + "", fixRateChargeIds) != -1) {
...
function calculateSlabPackingTypeRates(){
	var 	totalQty		= $('#totalQty').html();
	var chargeTypeId		= Number($('#chargeType').val());
	...
	if($('#wayBillType').val() != WayBillType.WAYBILL_TYPE_FOC) {
		var response = getSlabPackTyprRates(srcBranchId, destBranchId, vehicleTypeId, corporateAccountId, categoryTypeId, totalQty, rateTypeId, consigneeId);
		
		if(response && !jQuery.isEmptyObject(response)) {
			tempString		=	getTempStringForRates(response,totalQty);
		} 
	} 
	
	var sliptedByCommas		= new Array();
	sliptedByCommas			= tempString.split(",");

	for(var i=0;i<sliptedByCommas.length;i++) {
		chargeTypeId = sliptedByCommas[i].split("=")[0];
		if(chargeTypeId == BookingChargeConstant.DOOR_DLY_BOOKING) {
			wghtAmt = parseFloat(sliptedByCommas[i].split("=")[1]);
			$('#charge'+BookingChargeConstant.DOOR_DLY_BOOKING).val(wghtAmt);
		}
	}
}
...
//applied various condition for packing type rates form rate master
function getSlabPackTyprRates(srcBranchId, destBranchId, vehicleTypeId, corporateAccountId, categoryTypeId, userArticleInput, rateTypeId, consigneeId) {
