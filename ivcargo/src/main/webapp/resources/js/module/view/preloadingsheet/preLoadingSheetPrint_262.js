define(['elementTemplateJs'
	...
	return ({
		renderElements : function(response){
			...
			require(['text!/ivcargo/template/preloading/preloadingsheet_262.html'],function(PreLoadingSheet){
		...
		if (chunkArray[chunk].isTotalRow) {
			htmlVariable += '<div class="divTableRow">';
			htmlVariable += '<div class="divTableCell fontbold width200px borderLeft borderRight borderBottom borderTop" >' + chunkArray[chunk].TotalAmountLabel + '</div>';
			htmlVariable += '<div class="divTableCell fontbold width168px borderRight borderBottom borderTop">' + chunkArray[chunk].bookingTotal + '</div>';
			htmlVariable += '</div>';
			continue;
		 }
			...
			for(var index in sourceArray){
				...
				for(var destkey in detsinationcollection){
				   ...
				   branchTotalAmount += parseFloat(detsinationcollection[destkey].bookingTotal || 0);
				}
				
				finalJsonObj.push({
	                isTotalRow: true,
	                TotalAmountLabel: "Total",
	                bookingTotal: branchTotalAmount.toFixed(2)
                });
			}
		}
	});
