   ...
   /* ,JS_RESOURCES + '/resources/js/dashboard/html2canvas.min.js'*/
	...
	return Marionette.LayoutView.extend({
		initialize: function () {
			...
			getJSON(jsonObject, WEB_SERVICE_URL + '/deliveryMisDashboardWS/getDashboardDeliveryElementConfiguration.do?', _this.getElementConfigDetails, EXECUTE_WITHOUT_ERROR);
			return _this;
			...
			$("#mainContent").load("/ivcargo/html/dashboard/deliveryGrowth.html", function () {
				baseHtml.resolve();
			});
			...
			getJSON(jsonObject, WEB_SERVICE_URL + '/deliveryMisDashboardWS/getTheBarChartEvent.do', _this.setTheBarChartEvent, EXECUTE_WITH_ERROR);
			...
			years.forEach((d) => {
				...
				insidelist.forEach((insideData) => {
					...
					if (resultObject.dataType == "1")
						chartData.push(insideData.totalBookingTotal);
		   ...
		   if(thisMonthOrLastMonth){
			...
			_this.getTheglobalArraycheckboxvalue();
			  ...
			  getJSON(jsonObject, WEB_SERVICE_URL + '/deliveryMisDashboardWS/getTheChart.do', _this.printChartFirst, EXECUTE_WITH_ERROR);
		}
	});
});