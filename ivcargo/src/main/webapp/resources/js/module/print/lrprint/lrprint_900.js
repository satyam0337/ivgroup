    ...
    return ({
        renderElements : function(){
			...
			if(responseOut.wayBillTypeId == WAYBILL_TYPE_PAID)
				$(".hideChargesForPAID").hide();
					
			if(responseOut.wayBillTypeId == WAYBILL_TYPE_TO_PAY)
				$(".hideChargesForTOPAY").hide();
			
			if(responseOut.wayBillTypeId == WAYBILL_TYPE_CREDIT)
				$(".hideChargesForTBB").hide();
					

				$('#popUpContent900').bPopup({
				}, function() {
					var _thisMod = this;
					...
					$(this).html("<div class='confirm' style='height:150px;width:250px; padding:5px'>"
							...
							+"<div style='text-align:left;color:black;font-size:17px;'><input type='checkbox'  id='check' checked />&nbsp;<b style='font-size:14px;'>Consignor Details</b><div>"
