...
function getChargesRates() {
	...
	let isRouteWiseRate	= consignorPartyWayBillRates || consigneeWayBillRates || billingPartyWayBillRates;
	...
	if (configuration.CalculateCarrierRiskChargeOnDeclaredValue == 'true') {
		...
		if(configuration.applyOnlyPartyWiseLRLrLevelCharge == 'false' && !isRouteWiseRate && (configObject == null || configObject.length == 0)) {
...
function getRates(destBranchId, corporateAccountId, partyTypeId) {
	...
	$.getJSON("RouteAndLRLevelCharges.do?pageId=9&eventId=15",
...
function applyRates() {
	...
	setAOCcharge();
	calculateBranchWiseChargeBasedOnChargeTypeId();
	calcTotal();
}
...
function setAOCcharge(){
	var bookingDestBranchId    			= $('#destinationBranchId').val();
	var idsArr                          =  configuration.branchIdsForAOCCharge.split(",");
	
	if(configuration.destinationBranchWiseCharges || configuration.destinationBranchWiseCharges == 'true'){
		if(idsArr.length > 0){
			if(idsArr.includes(bookingDestBranchId)){
				if(configuration.amtPerArticleBranchwise > 0){
					$('#charge'+ChargeTypeMaster.AOC).val(getTotalAddedArticleTableQuantity() * configuration.amtPerArticleBranchwise);
					$('#charge'+ChargeTypeMaster.AOC).prop('readonly', true);
				}
			}
		}
	}
	...
	if(!isManual && configuration.isIncreaseCRInsuranceChargeAllow == 'true') {
		...
		if(bookingDestBranchId != undefined) {
			if (checkAddCarreirAllow(sourceBranchToAvoidArr,destBranchToAvoidArr,Number(bookingDestBranchId),destBranchToIncreaseArr)){
				var ele				= document.getElementById('charge' + BookingChargeConstant.CR_INSUR_BOOKING);
					if(ele) {
				...
				if(isAddCarreirAllow) {
					...
					if(totalQuantity > 0) {
						if(checkAddCrInsuranceQuantityWise()) {
							$('#charge' + BookingChargeConstant.CR_INSUR_BOOKING).val(Number(totalQuantity * amountToBeAddedInCRInsuranceCharge));
							$('#insuranceChargeValue').val(Number(totalQuantity * amountToBeAddedInCRInsuranceCharge));
							calcTotal();
						}else{
							$('#charge' + BookingChargeConstant.CR_INSUR_BOOKING).val(Number(amountToBeAddedInCRInsuranceCharge));
							$('#insuranceChargeValue').val(Number(amountToBeAddedInCRInsuranceCharge));
							calcTotal();
						}
						$('#charge' + BookingChargeConstant.CR_INSUR_BOOKING).removeAttr("readonly");
					}
				} else {
					if(configuration.isIncreaseCRInsuranceChargeAllow == 'true'){
						$('#charge' + BookingChargeConstant.CR_INSUR_BOOKING).val(0);
						$('#charge' + BookingChargeConstant.CR_INSUR_BOOKING).prop("readonly", "true");
					}
				}
...
function calculateBranchWiseChargeBasedOnChargeTypeId(){

	var	branchWiseChargesOnChargeIds 	= configuration.branchWiseChargesOnChargeId;
	var branchWiseChargesOnChargeIdArr	= branchWiseChargesOnChargeIds.split(",");
	var	sourceBranch 					= $('#sourceBranch').val();

	if(branchWiseChargesOnChargeIdArr != null && branchWiseChargesOnChargeIdArr.length > 0) {
		for (var i = 0; i < branchWiseChargesOnChargeIdArr.length; i++) {
			var branchIds			= branchWiseChargesOnChargeIdArr[i].split("_");

			if(branchIds[0] == sourceBranch || branchIds[0] == executive.branchId) {
				$("#charge" + branchIds[1]).val(branchIds[2]);
			}
		}
	}
}