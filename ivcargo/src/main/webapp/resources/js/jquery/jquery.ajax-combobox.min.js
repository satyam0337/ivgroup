...
/**
 * @desc To add a plug-in to the prototype of the jQuery
 * @global
 * @memberof jQuery
 * @param {string|Object} source - The server side of the program, or associative array itself .
 * @param {Object} option - Associative array that contains the option .
 * @param {boolean} [option.instance] - When you call a plug-in , whether to return an instance rather than the jQuery object
 * @param {string} [option.lang='ja'] - Language to be used in the plug -in message . ( 'en')
 * @param {string} [option.db_table='tbl'] - Database table to query .
 * @param {string} [option.field='name'] - Field to be used in the results display in the JavaScript side .
 * @param {string} [option.search_field=option.field] - Field name of the search target . That can be specified more than once in a comma-separated . (E.g .: 'id, name, job')
 * @param {string|Array} [option.order_by=option.search_field] - Serving as a reference field of rearrangement . It is acceptable in the array . (E.g .: 'name DESC' or [ 'name ASC', 'age DESC'])
 * @param {number} [option.per_page=10] - Number to display per page of the candidate list .
 * @param {number} [option.navi_num=5] - To display the page navigation , the number of adjacent page .
 * @param {boolean} [option.navi_simple=false] - Hide the link to the top - end of the page , to narrow as much as possible the width of the ComboBox.
 * @param {string} [option.plugin_type='combobox'] - 'combobox', 'simple', 'textarea'
 * @param {string} [option.init_record=false] - Record that specifies the initial value . To specify the value of the primary key .
 * @param {string} [option.bind_to] - The name of the event to be executed after the candidate selection
 * @param {string} [option.and_or='AND'] - AND Search , or OR search ('AND' or 'OR')
 * @param {boolean|string} [option.sub_info=false] - Display the sub-information. You can hide the item name by specifying a 'simple'. (True, false or 'simple')
 * @param {Objec} [option.sub_as={}] - Field alias of sub-information. Specified in the associative array.
 * @param {string} [option.show_field] - Fields to display in the sub- information . That can be specified more than once in a comma-separated . (e.g.: 'id' or 'id, job, age')
 * @param {string} [option.hide_field] - Field you want to hide in the sub-information. More can be specified in a comma-separated. (e.g.: 'id' or 'id, job, age')
 * @param {boolean} [option.select_only=false] - To select only. (Only accepted value registered in the database)
 * @param {string} [option.primary_key='id'] - When select only, it is specified in the value of the hidden of Form, fields that can identify the record.
 ...
 */
	var autoInstance = new Object();
	$.fn.ajaxComboBox = function(source, option) {
		var arr = [];
		this.each(function() {
			arr.push(new AjaxComboBox(this, source, option));
		})
		autoInstance[this.attr("id")] = arr 
	  return this;
	};

	$.fn.getInstance = function() {
		return $(autoInstance[this.attr("id")]);
	};
/**
 * @global
 * @constructor
 * @classdesc 要素ごとに適用される処理を集めたクラス
 * @param {Object} combo_input - HTML element to apply the plug-ins .
 * @param {string|Object} source - The server side of the program, or associative array itself .
 * @param {Object} option -Associative array that contains the option .
 */
...
$.extend(AjaxComboBox.prototype, /** @lends AjaxComboBox.prototype */ {
   ...
   * @private
   * @desc オプションの初期化
   * @param {string|Object} source - The path to the server-side language or data associative array of itself ,
  ...
  _setOption1st: function(source, option) {
    return $.extend({
      ...
      navi_num: 5,
      primary_key: 'id',
      button_img: 'glyphicon glyphicon-chevron-down',
      bind_to: false,
      navi_simple: false,
      ...
      // URL for shortening
      shorten_btn: false, // Selector of shortening the execution button
  ...
  /**
   * @private
   * @desc Option initialization the second stage of
   * @param {Object} option - Option in the form of an associative array
   * @return {Object} - Option second step is completed
   */
    ...
    switch (this.option.lang) {
      ...
      case 'en':
        message = {
          ...
          not_found   : 'not found',
          minCharac	  : 'Minimum'+ this.option.minLength +' characters.'
        };
        break;
    }
  ...
  _setCssClass: function() {
    ...
    var css_class = {
      ...
      re_off         : 'ac_results_off', //Candidate list ( non-selected state )
      select         : 'btn-primary', // In the selection<li>
    ...
    switch (this.option.plugin_type) {
      case 'combobox':
        css_class = $.extend(css_class, {
          ...
          btn_on  : 'btn btn-success', // ボタン(mover時)
          btn_out : 'btn btn-primary', // ボタン(mout時)
  ...
  _setProp: function() {
    this.prop = {
      timer_valchange: false, //  Timer variable (monitoring a change in the input value for each constant time)
      is_suggest: false, // Type of list. false => all cases / true => prediction
      page_all: 1,  // In the case of all cases display, the current page number
      page_suggest: 1, // Candidate display at the time of, the current page number
      max_all: 1,  // At the time of the Show All, the total number of pages
      max_suggest: 1, // Candidate display at the time of, the total number of pages
      is_paging: false, // Or page move?
      is_loading: false, // Whether in the inquiry in Ajax?
      reserve_btn: false, // Whether there is a reservation of the background color change of the button?
      reserve_click: false, // Detect the mousedown order to cope with the operation to continue to press the key of the mouse
      xhr: false, // Store XMLHttp object
      key_paging: false, // Whether you move the page with the key?
      key_select: false, // Or was the candidate move in key? ?
      prev_value: '', // initial value

      // Sub information
      size_navi: null, // Sub-information display (the height of the page navigation)
      size_results: null, // Sub-information display (top border of the list)
      size_li: null, // Sub-information display (the height of the candidate one line)
      size_left: null, // Sub-information display (horizontal width of the list)
  ...
  /**
   * @private
   * @desc Create an element, to generate HTML
   * @param {Object} combo_input - Targeted by HTML elements of the plug-in application
   */
  _setElem: function(combo_input) {
    // 1. Create an element, to generate
    // Body
    var elem = {};
    ...
    if (this.option.plugin_type == 'combobox') {
      ...
      elem.img    = $('<i>').attr('class', this.option.button_img);
    } else {
      elem.button = false;
      elem.img    = false;
    }
    ...
    elem.navi_info   = $('<div>').addClass('naviinfo');
    elem.navi_p      = $('<p>');
  ...
  /**
   * @private
   * @desc Determine the initial state of the screen display
   */
  _setButtonAttrDefault: function() {
    if (this.option.select_only) {
      if ($(this.elem.combo_input).val() !== '') {
        ...
        // Return to the complete initial state
    ...
    if (this.option.plugin_type == 'combobox') {
      ...
      $(this.elem.button).attr('data-toggle', "tooltip");
      $(this.elem.button).attr('data-placement', "top");
      $(this.elem.button).attr('data-container', "body");
      $(this.elem.img).attr('class', this.option.button_img);
    }
  },
  ...
  _setInitRecord: function() {
    ...
    // To insert the value of to select only
    // Insert the value to hidden
    if (this.option.plugin_type != 'textarea') $(this.elem.hidden).val(this.option.init_record);
    // Insert the value to the text box
    if (typeof this.option.source == 'object') {
      // If the source is a data set
      var data;
  ...
  /**
   * @private
   * @desc Candidate list: from, entry: clarity, the sub-information: Hide
   * @param {Object} self -Reference to an instance object of this class
   */
  ...
  /**
   * @private
   * @desc Candidate list: clarity, entry: black
   * @param {Object} self - Reference to an instance object of this class
   */
  ...
  /**
   * @private
   * @desc URLTo enable the shortening button
   * @param {Object} self - Reference to an instance object of this class
   */
  ...
  _searchForDb: function(self, q_word, which_page_num) {
	  //For All or down key and if Entered should be greater than minLength
	  if(self.elem.combo_input.context.value.length== 0 
			  || self.elem.combo_input.context.value.length >= self.option.minLength){
		  self.prop.xhr = $.ajax({
			  dataType: 'json',
			  url: self.option.source,
			  data: {
		  ...
		  self.prop.xhr = null;
		  self._minimumCharactersToEnter(self);
		  return;
	  }
  },
  ...
  _searchForJson: function(self, q_word, which_page_num) {
    ...
    // 見つからなければすぐに終了
    if (matched.length === undefined || matched.length <= 0) {
      self._notFoundSearch(self);
      return;
    }
  ...
  _minimumCharactersToEnter: function(self) {
	  $(self.elem.navi_info).text(self.message.minCharac);
	  $(self.elem.navi_p).hide();
	  $(self.elem.results).empty();
	  $(self.elem.sub_info).empty();
	  self._calcWidthResults(self);
  ...
  _displayResults: function(self, arr_candidate, arr_subinfo, arr_primary_key) {
    ...
    $(self.elem.results).empty();
    $(self.elem.sub_info).empty();
    var candidateWithZero = new Object();
    var checkForAll = true;
    for (var i = 0; i <= arr_candidate.length; i++) {

    	if(arr_candidate.length == 1){
    	}
    	if(arr_primary_key[i] == self.option.valueAtLast){
    		candidateWithZero.primary_key = arr_primary_key[i];
    		candidateWithZero.candidate = arr_candidate[i];
    		checkForAll = false;
    		continue;
    	}
    	if(i == arr_candidate.length){
    		 // 候補リスト
    		if(checkForAll){
    			continue;
    		}
    	      var list = $('<li>')
    	        .text(candidateWithZero.candidate) // XSS対策
    	        .attr({
    	          pkey: candidateWithZero.primary_key,
    	          'data-toggle':'tooltip',
    	          'data-placement':'top',
    	          'data-container':'body',
    	          title: candidateWithZero.candidate
    	        });
    	}else{
    		// 候補リスト
    		var list = $('<li>')
  ...
  _selectCurrentLine: function(self, is_enter_key) {
    // 選択候補を追いかけてスクロール
    //$(self.elem.combo_input[0]).attr("value",$(self.elem.hidden[0]).val());
	  self._scrollWindow(self, true);
    ...
    $(self.elem.combo_input).focus();  // Move the focus to the text box
    $(self.elem.combo_input).change(); // Notice that the value of the text box has changed
    self._setCssFocusedInput(self);
    //$(self.elem.combo_input[0]).attr("data-autocompleteid",""+$(self.elem.hidden[0]).val());
     $( '#'+self.elem.combo_input.context.id ).trigger( "click" );
  },
