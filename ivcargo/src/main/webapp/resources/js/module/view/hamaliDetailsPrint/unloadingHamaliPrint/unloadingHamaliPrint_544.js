...
var executive = response.executive;
	...
	$("#mainContent").load("/ivcargo/html/module/hamaliDetails/unLoadingHamaliPrint/unLoadingHamaliPrint.html",function() {
	baseHtml.resolve();
});
...
var hamaliDetailsList = response.receivedDetailsListFinal;
var hamaliDetails = response.hamaliDetails;
var columnArray = new Array();
var count = 0;

console.log('---',hamaliDetailsList.length)
...
for (var i = 0; i < hamaliDetailsList.length; i++) {
				...
				$('#lsNumber').html(obj.turNumber);
				var amount = (hamaliDetails.unLoadingPerArticle*obj.receivedQuantity);
				count =count + 1;
				totalQty+=obj.receivedQuantity;
				totalAmount	+= amount;
				...
				columnArray.push("<td class='datatd' style='text-align: center; vertical-align: middle; font-size:15px;''>"+obj.receivedQuantity+"</td>");
				columnArray.push("<td class='datatd' style='text-align: center; vertical-align: middle; font-size:15px;''>"+obj.weight+"</td>");
			//	columnArray.push("<td class='datatd' style='text-align: center; vertical-align: middle; font-size:15px;''>"+hamaliDetails.unLoadingPerArticle+"</td>");
				columnArray.push("<td class='datatd' style='text-align: center; vertical-align: middle; font-size:15px;''>"+amount+"</td>");
