...
function checkIfnotPresent() {
	...
	if(fixedChargeRateObj[BookingChargeConstant.FREIGHT] != null && fixedChargeRateObj[BookingChargeConstant.FREIGHT])
		chargedWeight	= 1;
...
function getChargesRates() {
	...
	configObject = filterLRLevelRates(3, branchId, destBranchId, chargeConfigPartyId, 0, 0, 0);
		...
		if(rateMaster != null && rateMaster.length > 0) {
			for (var i = 0; i < rateMaster.length; i++) {
				if(rateId_mast[rateMaster[i].chargeTypeMasterId] == null || (typeof rateId_mast[rateMaster[i].chargeTypeMasterId] == 'undefined') || rateId_mast[rateMaster[i].chargeTypeMasterId] == 0) {
					...
					fixedChargeRateObj[rateMaster[i].chargeTypeMasterId] = rateMaster[i].isFixedSlabAmt && rateMaster[i] && rateMaster[i].rate > 0;
				} else {
					if(parseFloat(userInput) >= rateMaster[i].minWeight && parseFloat(userInput) <= rateMaster[i].maxWeight) {
...
function calculateWeightTypeRates(chargedWeight, typeofPacking, consignmentGoodsId) {
	...
	var corporateAccountId	= 0;
	var categoryTypeId		= RateMaster.CATEGORY_TYPE_GENERAL_ID;
	...
	if($('#wayBillType').val() != WayBillType.WAYBILL_TYPE_FOC) {
		var response =  getWghtTypeRates(srcBranchId, destBranchId, vehicleTypeId, corporateAccountId, categoryTypeId, typeofPacking, packingGrpTypeId, chargedWeight, 0, consignmentGoodsId, consigneeId);
		...
		if(response && !jQuery.isEmptyObject(response)) {
			...
			if(weightFromDb > 0) {
				...
				if(fixedChargeRateObj[BookingChargeConstant.FREIGHT] != null && fixedChargeRateObj[BookingChargeConstant.FREIGHT])
					weightAmount	= weightFromDb;
				else if(weightTypeForRateApply == ChargeTypeConstant.WEIGHT_TYPE_ID_ACTUAL_WEIGHT)
...
function calcGrandtotal() {
	...
	if(configuration.doNotCalculateAutomaticTax == 'false' || configuration.doNotCalculateAutomaticTax == false)
		...
		grandtotal		= grandtotal + Number($('#tax5').val());
...
function calculateTaxManually(obj) {
	//get total charge amount
	var amount				= getAmountToCalculateEncludedTax();
	
	//get service tax excluded amount
	var serviceTaxExclude 	= getServiceTaxExcludeCharges();
	...
	//calculate tax
	var taxAmount			= calculateVATTax(jsondata.taxes, grandtotal, discAmount);
	
	if(Number(obj.value) > taxAmount) {
		var taxes			= jsondata.taxes;
		var perTaxAmount	= taxes[0].taxAmount;
		
		if (taxes[0].isTaxPercentage)
			showMessage('error', 'VAT amount ' + obj.value + ' cannot be greater than ' + perTaxAmount + '% (' + taxAmount + ') of amount ' + amount);
		else
			showMessage('error', 'VAT amount ' + obj.value + ' cannot be greater than ' + perTaxAmount + ' !');
		
		setTimeout(function(){ $("#tax5").focus(); }, 10);
		obj.value = 0;
	}
	
	grandtotal	= Number(obj.value) + grandtotal;
