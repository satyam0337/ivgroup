...
define([
        ...
        ,PROJECT_IVUIRESOURCES+'/resources/js/module/view/dispatchdestinationwisesummary/loaddispatchdestinationwisesummarymodelurls.js'
        ,'elementmodel'
        ,'slickGridWrapper'
        ,'errorshow'
        ,'messageUtility'
	...
	return Marionette.Behavior.extend({
		...
		onSearchLRDetails: function() {
			...
			//alert("destSub "+$('#destBranchSelectEle_primary_key').val()+" branch "+$('#destSubRegionSelectEle_primary_key').val());

			if($('#destBranchSelectEle_primary_key').val() != undefined) {
			jsonObject["destinationBranch"] 			= $('#destBranchSelectEle_primary_key').val();
		}
		if($('#destSubRegionSelectEle_primary_key').val() != undefined) {
			jsonObject["destinationSubRegionId"] 		= $('#destSubRegionSelectEle_primary_key').val();
		}
			...
			getJSON(jsonObject, WEB_SERVICE_URL+'/destinationWiseDispatchSummaryReportWS2/getDestinationWiseDispatchSummaryDetails.do', _this.setData, EXECUTE_WITH_ERROR);
