...
var chromeplugin = (function() {
  ...
  function runTests(moduleEl) {
    ...
    chromeplugin.tester = new Tester();
    ...
    // has crashed.
    chromeplugin.tester.exitCleanlyIsOK();

    chromeplugin.tester.addAsyncTest('loaded', function(test) {
      test.pass();
    });
    ...
    chromeplugin.tester.waitFor(moduleEl);
    chromeplugin.tester.run();
  }
  ...
  function createNaClModule(name, tool, path, width, height, attrs) {
	...
	//moduleEl.setAttribute('src', 'pluginfiles/' + name + '.nmf');
    moduleEl.setAttribute('src', 'plugins/Machine_Locking/Chrome/pluginfiles/' + name + '.nmf');
  ...
  function handleError(event) {
    // We can't use chromeplugin.naclModule yet because the module has not been
    // loaded.
  ...
  function handleCrash(event) {
    if (chromeplugin.naclModule.exitStatus == -1) {
      updateStatus('CRASHED');
    } else {
      updateStatus('EXITED [' + chromeplugin.naclModule.exitStatus + ']');
    }
    if (typeof window.handleCrash !== 'undefined') {
      window.handleCrash(chromeplugin.naclModule.lastError);
    }
  }
  ...
  function moduleDidLoad() {
    chromeplugin.naclModule = document.getElementById('nacl_module');
   ...
   */
  function hideModule() {
    // Setting chromeplugin.naclModule.style.display = "None" doesn't work; the
    // module will no longer be able to receive postMessages.
    chromeplugin.naclModule.style.height = '0';
  }
  ...
  function removeModule() {
    chromeplugin.naclModule.parentNode.removeChild(chromeplugin.naclModule);
    chromeplugin.naclModule = null;
  }
  ...
  function domContentLoaded(name, tool, path, width, height, attrs) {
    ...
    } else if (chromeplugin.naclModule == null) {
...
document.addEventListener('DOMContentLoaded', function() {
  var body = document.getElementById('chromeplugin'); //body;

  // The data-* attributes on the body can be referenced via body.dataset.
  if (body.dataset) {
    var loadFunction;
    if (!body.dataset.customLoad) {
      loadFunction = chromeplugin.domContentLoaded;
