define(['marionette'
        ...
        function(Marionette, UrlParameter, html2canvas) {
	...
	return Marionette.LayoutView.extend({
		initialize: function() {
			...
			getJSON(jsonObject, WEB_SERVICE_URL + '/lrPrintWS/tranceLRPrint.do?',_this.getData, EXECUTE_WITHOUT_ERROR);
			...
			require(['/ivcargo/resources/js/module/print/tceLrPrintSetup.js'], function(lrprintsetup) {
					require(['text!' + lrprintsetup.getConfiguration(),
				    lrprintsetup.getFilePathForLabel()], function(View, FilePath) {
					_this.$el.html(_.template(View));
					lrprintsetup.setHeader(responseOut);
					lrprintsetup.setLRDetails(responseOut);
					lrprintsetup.setConsignee(responseOut.ConsigneeDetails);
					lrprintsetup.setConsignor(responseOut.ConsignorDetails);
					lrprintsetup.setConsignmentDts(responseOut.CONSIGNMENTDETAILS);
    				lrprintsetup.setCharge(responseOut);
					lrprintsetup.setCharge(responseOut);
					lrprintsetup.setFooter(responseOut)
					
					setTimeout(function() {
						//lrprintsetup.sendWhatsAppPdf(responseOut);
						window.print();
					}, 200);
				})
			});
