...
define([
	 PROJECT_IVUIRESOURCES + '/resources/js/module/view/partyWiseShortCreditPaymentReport/partyWiseShortCreditPaymentReportLRDatafilepath.js'
	,'jquerylingua'
	...
	return Marionette.LayoutView.extend({
		...
		render: function(){
			getJSON(jsonObject, WEB_SERVICE_URL+'/partyWiseShortCreditPaymentReportWS/getPartyWiseShortCreditPaymentReportLRWiseData.do', _this.setElementData, EXECUTE_WITH_ERROR);
		},setElementData : function(response) {

			$("#partyWiseShortCreditPaymentReportLRDiv").empty();
			...
			$("#mainContent").load("/ivcargo/html/report/partyWiseShortCreditPaymentReport/partyWiseShortCreditPaymentReportLRData.html",function() {
				baseHtml.resolve();
			});
			...
			$.when.apply($, loadelement).done(function() {
				...
				if(response.ShortCreditPaymentRegister.CorporateAccount != undefined && response.ShortCreditPaymentRegister.CorporateAccount.length > 0) {
					...
					for(var i=0;result.length > i;i++) {
						...
						var wayBillNumber							= result[i].wayBillNumber;
						result[i].wayBillNumberWithoutBranchCode	= Number(wayBillNumber.substr(wayBillNumber.indexOf("/") + 1));
					}

					response.ShortCreditPaymentRegister.CorporateAccount = _.sortBy(result,'wayBillNumberWithoutBranchCode');
