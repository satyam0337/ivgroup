...
function setRatesTableWise() {
	...
	for (let k in consignmentDataHM) {
		...
		//calculateWeightTypeRates(chargedWeight,typeofPacking);
		getWeightTypeRates();
		checkIfnotPresent();
...
function editQuantityAmount() {
	...
	if(Number(quantityAmount) < Number(qtyAmt)) {
	...
	if(getBookingType() == BookingTypeConstant.BOOKING_TYPE_FTL_ID) {
		...
		if(vehicleTypeValue != 0) {
			...
			if(parseFloat(actualWeight) > parseFloat(val[1])) {
				isWeightAllow	= configuration.FTLWeightAllowMoreThenTruck == 'true';
			}
		} else {
...
function checkPackingTypeAllowedOnWeight() {
	if(configuration.isCheckPackingTypeOnWeight == 'true')
		return checkPackingTypeTableWise();
...
function checkPackingTypeTableWise() {
	var actualWeight	= $('#actualWeight').val();
	
	if(actualWeight <= configuration.weightLimit) {
		for (let k in consignmentDataHM) {
			let typeofPacking 	= consignmentDataHM[k].packingTypeMasterId;
			let packingIdList 	= (configuration.allowedPackingTypeId).split(',');
			
			if(!packingIdList.includes(typeofPacking)) {
				showMessage('info', packingTypeErrorOnWeight);
				setTimeout(function(){$('#actualWeight').focus(); }, 10);
				return false;
			}
		}
	}
...
function checkPackingTypeAllowedOnTempWeight(){

	if(configuration.isCheckPackingTypeOnWeight == 'true') {
		var tempWeight	= $('#tempWeight').val();
		if(tempWeight <= configuration.weightLimit){
			var typeofPacking	= 0;
			...
			var packingIdList 		= (configuration.allowedPackingTypeId).split(',');
			
			if(!packingIdList.includes(typeofPacking)){
				showMessage('info', packingTypeErrorOnWeight);
				setTimeout(function(){$('#tempWeight').focus(); }, 10);
				return false;
			}
		}
	}
...
function getPackingTypeRates() {
	...
	if(configuration.SubRegionWiseRateRequired == 'true'){
		var destSubRegionId 	= $('#destinationSubRegionId').val();
		var subRegionWithWithPagkingGroupWiseRate	= configuration.subRegionRateValues;
		var subRegionRateList	= subRegionWithWithPagkingGroupWiseRate.split(',');
		...
		if(checkData)
			configuration.setConstantArticleAmount = false;
	}
...
function applyRates() {
	...
	applyExtraCharges();
	calculateFreightForOverNightFrghtCarriers();
...
function validateArticleAmountWithWeight(){
	...
	var wbType					= parseInt($('#wayBillType').val());

	if( wbType == WayBillType.WAYBILL_TYPE_FOC) {
		return true;
	}

	if (configuration.TemporaryWght == 'true') {
		if(!validateInput(1, 'tempWeight', 'tempWeight', 'packageError',  tempWeightErrMsg)) {
			return false;
		}
	}
	...
	if(configuration.SubRegionWiseRateRequired == 'true'){
		...
		var destSubRegionId 	= $('#destinationSubRegionId').val();
		var subRegionWithWithPagkingGroupWiseRate	= configuration.subRegionRateValues;
		var subRegionRateList	=	new Array();
		subRegionRateList 			= subRegionWithWithPagkingGroupWiseRate.split(',');
		var subRegionList			= new Array();
		...
		var checkData = isValueExistInArray(subRegionList,destSubRegionId)
		if(checkData){
			configuration.validateArticleAmountWithWeight = false;
		}
		
	}
	
	var partyRates = new Array();
	
	if(wayBillRates != null){
		for(var i = 0;i<wayBillRates.length;i++){
			if($("#billingPartyId").val() > 0){
				if(((wayBillRates[i].wayBillTypeId == $('#wayBillType').val()) 
						&& wayBillRates[i].destinationBranchId == $('#destinationBranchId').val()) 
						&& (wayBillRates[i].chargeTypeId == ChargeTypeConstant.CHARGETYPE_ID_WEIGHT) 
						&&  (wayBillRates[i].corporateAccountId == $('#billingPartyId').val())){
					partyRates.push(wayBillRates[i]);
				}
			} else{
				if(((wayBillRates[i].wayBillTypeId == $('#wayBillType').val()) 
						&& wayBillRates[i].destinationBranchId == $('#destinationBranchId').val()) 
						&& (wayBillRates[i].chargeTypeId == ChargeTypeConstant.CHARGETYPE_ID_WEIGHT) 
						&&  (wayBillRates[i].corporateAccountId == $('#partyMasterId').val())){
					partyRates.push(wayBillRates[i]);
				}
			}
		}
	...
	if(configuration.validateArticleAmountWithWeight == 'true'  && $('#chargeType').val() == ChargeTypeConstant.CHARGETYPE_ID_QUANTITY) {
		if (configuration.validateConstantWeightForChargeType == 'true' && Number($('#tempWeight').val()) > Number(configuration.minWeightForArtWeightCheck)) {
			showMessage('info', WeightRateErrMsg);
			return false;
		} else {
			var packingTypeIds	= (configuration.packingTypeIdsForsetConstantArtAmount).split(",");
			var typeofPacking	= 0;
			...
			for(var j=0; j < packingTypeIds.length; j++) {

				if(Number(packingTypeIds[j]) == Number(typeofPacking)) {

					var minMaxWeightAndAmountArr = (configuration.minMaxWeightAndAmountForValidateArtAmount).split(",")

					for(var k=0; k < minMaxWeightAndAmountArr.length; k++) {

						var min = (minMaxWeightAndAmountArr[k]).split("_")[0];
						var max = (minMaxWeightAndAmountArr[k]).split("_")[1];
						var Amt = (minMaxWeightAndAmountArr[k]).split("_")[2];

						if (Number($('#tempWeight').val()) >= Number(min) && Number($('#tempWeight').val()) <= Number(max)) {

							if(Number(qtyAmount) != Number(Amt)) {
								showMessage('info', ArticleWeightErrMsg);
								return false;
							}
						}
					}
	...
	} else if(configuration.validateConstantWeightForChargeType == 'true' && $('#chargeType').val() == ChargeTypeConstant.CHARGETYPE_ID_WEIGHT) {
		if (Number($('#tempWeight').val()) <= Number(configuration.minWeightForArtWeightCheck) && !(partyRates.length > 0)) {
			showMessage('info', WeightRateErrMsg);
			return false;
		}
	}
...
function calculateFreightForOverNightFrghtCarriers(){
	if(configuration.SubRegionWiseRateRequired == 'true'){
		var typeofPacking = 0;
		
		if($('#typeofPacking').val() > 0) {
			typeofPacking		= $('#typeofPacking').val();
		} else if($('#typeofPackingId').val() > 0){
			typeofPacking		= $('#typeofPackingId').val();
		} else {
			typeofPacking		= $('#typeofPackingId1').html();
		}
		...
		if(checkData){
			for(var i = 0;i<subRegionRateList.length;i++){
				var subRegionRateData = subRegionRateList[i].split('_');
				
				if(subRegionRateData[0] == destSubRegionId){
					for(var i = 0; i < PackingGroupMappingArr.length; i++){
						//PackingGroup - for Konduskar packing Group 	Document	2
						//PackingGroup - for Konduskar packing Group 	NonDocument	3
						if(PackingGroupMappingArr[i].packingTypeMasterId == typeofPacking && PackingGroupMappingArr[i].packingGroupTypeId == 2) {
							var tempWeight		= $('#tempWeight').val();
							//var frghtVal	=	$('#charge'+BookingChargeConstant.FREIGHT).val();
							var quotient		= Math.ceil(tempWeight/0.250);
							$('#charge'+BookingChargeConstant.FREIGHT).val(quotient * subRegionRateData[1]);
						} else if (PackingGroupMappingArr[i].packingTypeMasterId == typeofPacking && PackingGroupMappingArr[i].packingGroupTypeId == 3){
							var tempWeight		= $('#tempWeight').val();
							if(tempWeight <= 3){
								//var frghtVal	=	$('#charge'+BookingChargeConstant.FREIGHT).val();
								$('#charge'+BookingChargeConstant.FREIGHT).val(3 * subRegionRateData[2]);
							} else{
								//var frghtVal	=	$('#charge'+BookingChargeConstant.FREIGHT).val();
								$('#charge'+BookingChargeConstant.FREIGHT).val(tempWeight * subRegionRateData[2]);
							}
						}
					}
				}
