...
function printLhpvWindow() {
	$("#tableContain").load( "/ivcargo/jsp/print/lhpv/399/aptLHPVPrint.html", function() {
		printAfterDelay();
		if(isShowBankDetails == true || isShowBankDetails == 'true'){
...
function setDataInTable() {
	...
	setValueToHtmlTag('bankName', vehicleNumberMaster.bankName);
	setValueToHtmlTag('registeredOwner', vehicleNumberMaster.registeredOwner);
	setValueToHtmlTag('accountNo', vehicleNumberMaster.accountNo);
	setValueToHtmlTag('ifscCode', vehicleNumberMaster.ifscCode);
	setValueToHtmlTag('bankBranchName', vehicleNumberMaster.bankBranchName);
...
function setCompanyDetails(companyName) {
	companyName = 'PRAGATI ROAD CORPORATION (VJW)';
...
function calculateCharges(){
	...
	if(lhpvChargesAmt != null) {
		...
		for(var key in lhpvChargesAmt) {
			...
			else if(key == LHPVChargeTypeConstant.UNLOADING_HAMALI) {
				lhpvSettlementCharges	= lhpvChargesAmt[key];
				unloadingHamali			= lhpvSettlementCharges.chargeAmount;
			} else if(key == LHPVChargeTypeConstant.LOADING_HAMALI) {
				lhpvSettlementCharges	= lhpvChargesAmt[key];
				loadingHamali			= lhpvSettlementCharges.chargeAmount;
			} 
		}
		total   = Math.round(lorryHireAmt + hamali + oh + ol + lc + doorDelivery + crossingLH + crossingHamali + cartage + other1 + co_LH + loadingHamali + unloadingHamali);
	...
	return {
			...
			other2:other2,
			ratepmt:ratepmt,
			loadingHamali:loadingHamali,
			unloadingHamali:unloadingHamali
	}
}
...
function setCharges() {
	var chargesColl = calculateCharges();
		...
		if(chargesColl.loadingHamali != 0) {
			createTableRow("Loading Hamali", chargesColl.loadingHamali, 'chrgTable');
		}
		if(chargesColl.unloadingHamali != 0) {
			createTableRow("Unloading Hamali", chargesColl.unloadingHamali, 'chrgTable');
		}
...
function showBoliWeight(){
	$(".showBoliWgt").show();
}

function showActualWeight(){
	$(".showActualWgt").show();
}

function hideActualWeight(){
	$(".hideActualWgt").hide();
}

function hideBoliWeight(){
	$(".hideBoliWgt").hide();
}
$(document).ready(function(){
	 document.getElementById("showActualWgt").checked = true;
	 document.getElementById("showBoliWgt").checked = true;
});