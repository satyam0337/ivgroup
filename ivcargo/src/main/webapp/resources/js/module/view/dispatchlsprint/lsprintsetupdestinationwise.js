	...
	return {
		setDefaultPrint:function(responseObj) {
			...
			_.mapObject(groupDataObject,function(groupData,key){
				finalDivObj.append(_this.setHeaderWiseData(groupData,dispatchDataAccountGroupWise[key],dispatchLSLRCharge,configuration,langKeySetLRSummary,dispatchLSHeaderAccountGroupWise[key],summaryConfiguration,dispatchLSLRFormSummaryAccountGroupWise[key],LSLRFormDetailsConfig,LHPVSummaryConfig,showPartialLSDetailsSummary,partialLSconfiguration))
			})
...
//			$("*[data-dataCell=wayBillArticleQuantity]").addClass("rightCell");
		...
		plainPrint:function(copyDataForPrint) { // Function For Dot Matrix Printer Print. It Will Remove CSS Class
		...
		},setHeaderWiseData:function(headerCollection,contentCollection,dispatchLSLRCharge,configuration,langKeySetLRSummary,dispatchLSHeader,summaryConfiguration,dispatchLSLRFormSummary,LSLRFormDetailsConfig,LHPVSummaryConfig,showPartialLSDetailsSummary,partialLSconfiguration){
			var _this = this;
			var divObj = $('<div/>')
			...
			_.each(splittedVal,function(pageWiseContent){
				_this.setHeader(headerCollection,divObj,dispatchLSHeader)
				...
				_.each(pageWiseContent,function(lrDataContent){
					if(lrDataContent['destinationBranchName'] != undefined){
						...
						totalvasuliAmount		+=	parseFloat(dataObj.wayBillVasuliAmount);
						totalotherAmount		+= 	parseFloat(dataObj.wayBillOtherAmount);
						totalserviceTaxAmount	+= 	parseFloat(dataObj.wayBillServiceTaxAmount);
						totalcarrierRiskAmount	+=	parseFloat(dataObj.wayBillCarrierRiskAmount);
			...
			var totalSummaryTable = _this.setLSTotalSummary(finalObj,dispatchLSHeader,headerCollection,summaryConfiguration,dispatchLSLRCharge,dispatchLSLRFormSummary,LSLRFormDetailsConfig,LHPVSummaryConfig);
			divObj.append(totalSummaryTable);
			if(showPartialLSDetailsSummary == true){
				var partialLSPrintLRSummary	= _this.setPartialLRSummary(partialCollection, partialLSconfiguration.partialLSconfiguration, dispatchLSHeader, headerCollection, dispatchLSLRCharge); // To Set LR Summary
				divObj.append(partialLSPrintLRSummary);
		...
		setHeader:function(headerCollection,divObj,dispatchLSHeader){
			...
			dataDiv.push("To :"+headerData[0].dispatchLSDestinationBranchName); // Providing Dat For Div i.e. Destination Branch Name
