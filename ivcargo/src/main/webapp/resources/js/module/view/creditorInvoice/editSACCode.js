			...
			$(".ok").on('click', function() {
				...
				if(myNod.areAll('valid')) {
					...
					if(bill.billBillId > 0) {
						...
						jsonObjectNew["previousSACCode"] 			= bill.previousSACCode;
						jsonObjectNew["sacCode"] 					= $('#sacCode').val();
						jsonObjectNew["previousExecutiveId"] 		= bill.billExecutiveId;
					}
	
					btModalConfirm = new Backbone.BootstrapModal({
						content		: 	"Are you sure you want to Update SAC Code?",
						modalWidth 	: 	30,
						title		:	'Update SAC Code',
						okText		:	'YES',
						showFooter 	: 	true,
						okCloses	:	true
					}).open();
					...
					btModalConfirm.on('ok', function() {
						...
						getJSON(jsonObjectNew, WEB_SERVICE_URL+'/creditorInvoiceWS/updateBillSACCode.do', _this.onUpdate, EXECUTE_WITH_ERROR); //submit JSON
						showLayer();
					});
				} else {
					return false;
