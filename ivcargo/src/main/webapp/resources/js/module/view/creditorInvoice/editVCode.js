			...
			$(".ok").on('click', function() {
				...
				if(myNod.areAll('valid')) {
					...
					if(bill.billBillId > 0) {
						...
						jsonObjectNew["previousVCode"] 				= bill.previousVCode;
						jsonObjectNew["vCode"] 						= $('#vCode').val();
					}
	
					btModalConfirm = new Backbone.BootstrapModal({
						content		: 	"Are you sure you want to Update V Code?",
						modalWidth 	: 	30,
						title		:	'Update V Code',
						okText		:	'YES',
						showFooter 	: 	true,
					...
					btModalConfirm.on('ok', function() {
						...
						getJSON(jsonObjectNew, WEB_SERVICE_URL+'/creditorInvoiceWS/updateBillVCode.do', _this.onUpdate, EXECUTE_WITH_ERROR); //submit JSON
						showLayer();
					});
				} else {
					return false;
