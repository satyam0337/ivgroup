...
var wordFound = false;  //Boolean variable to check if word found or not
...
function actb(obj,ca){
	...
	function actb_penter(){
		...
		for (var i=0;i<=actb_self.actb_keywords.length;i++){
			if (actb_bool[i]) c++;
			if (c == actb_pos){
				stringId=actb_self.actb_keywords[i][0];
				word = actb_self.actb_keywords[i][1];
				break;
			}
		}
		actb_insertword(word);
		// Use following function for other operatons while inserting the word
		if(typeof (window.getSeletedItemData) == 'function') {
			// function exists, so we can now call it
			getSeletedItemData(actb_keywords[i]);
			}
